...
function setupIPC() {
  ...
  // ── JOBS ──
  safeHandle("jobs:list", (_, { limit = 200, offset = 0, search = "", closingMonth = "" } = {}) => {
    let sql = "SELECT * FROM jobs WHERE 1=1";
    const params = [];
    if (search) { const q = `%${search}%`; sql += " AND (id LIKE ? OR bl LIKE ? OR mbl LIKE ? OR customer LIKE ?)"; params.push(q, q, q, q); }
    if (closingMonth) { sql += " AND substr(closingDate,1,7)=?"; params.push(closingMonth); }
    sql += " ORDER BY createdAt DESC LIMIT ? OFFSET ?";
    params.push(limit, offset);
    return db.prepare(sql).all(...params).map(r => rowToObj("jobs", r));
  });
  safeHandle("jobs:count", (_, { search = "", closingMonth = "" } = {}) => {
