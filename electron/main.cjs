...
const JSON_COLS = {
  ...
  invoices: ["items", "displayFields", "jobData", "payments", "invoicedCodes", "taxComponents", "consolidatedFrom"],
  apRecords: ["items", "payments", "taxPaidComponents"],
};
...
function runMigrations() {
  ...
  if (ver < 11) {
    try { db.exec("ALTER TABLE apRecords ADD COLUMN taxPaid REAL DEFAULT 0"); } catch (_) {}
    try { db.exec("ALTER TABLE apRecords ADD COLUMN taxPaidComponents TEXT DEFAULT '[]'"); } catch (_) {}
    setVer(11);
  }
  if (ver < 12) {
    try { db.exec("ALTER TABLE invoices ADD COLUMN consolidatedFrom TEXT DEFAULT '[]'"); } catch (_) {}
    try { db.exec("ALTER TABLE invoices ADD COLUMN consolidatedInto TEXT DEFAULT ''"); } catch (_) {}
    setVer(12);
  }
}
...
function setupIPC() {
  ...
  // ── JOBS ──
  safeHandle("jobs:list", (_, { limit = 200, offset = 0, search = "", createdMonth = "", createdFrom = "", createdTo = "" } = {}) => {
    let sql = "SELECT * FROM jobs WHERE 1=1";
    const params = [];
    if (search) { const q = `%${search}%`; sql += " AND (id LIKE ? OR bl LIKE ? OR mbl LIKE ? OR customer LIKE ? OR cb LIKE ? OR ff LIKE ?)"; params.push(q, q, q, q, q, q); }
    if (createdMonth) { sql += " AND substr(createdAt,1,7)=?"; params.push(createdMonth); }
    if (createdFrom) { sql += " AND createdAt>=?"; params.push(createdFrom + "T00:00:00"); }
    if (createdTo) { sql += " AND createdAt<=?"; params.push(createdTo + "T23:59:59"); }
    sql += " ORDER BY createdAt DESC LIMIT ? OFFSET ?";
    params.push(limit, offset);
    return db.prepare(sql).all(...params).map(r => rowToObj("jobs", r));
  });
  safeHandle("jobs:count", (_, { search = "", createdMonth = "", createdFrom = "", createdTo = "" } = {}) => {
  ...
  safeHandle("jobs:delete", (_, id) => {
    const job = db.prepare("SELECT * FROM jobs WHERE id=?").get(id);
    if (!job) return { success: false, error: "Job not found" };
    const parsed = rowToObj("jobs", job);
    const hasInv = (parsed.cbInvoices || []).length > 0 || (parsed.ffInvoices || []).length > 0;
    if (hasInv) return { success: false, error: "Cannot delete BL with invoices" };
    db.prepare("DELETE FROM jobs WHERE id=?").run(id);
    db.prepare("DELETE FROM apRecords WHERE jobId=?").run(id);
    return { success: true };
  });
  ...
  safeHandle("dashboard:stats", () => {
    const totalBLs = db.prepare("SELECT COUNT(*) as c FROM jobs").get().c;
    const openBLs = db.prepare("SELECT COUNT(*) as c FROM jobs WHERE closedMonth='' OR closedMonth IS NULL").get().c;
    const closedBLs = totalBLs - openBLs;
    const totalInvs = db.prepare("SELECT COUNT(*) as c FROM invoices WHERE consolidatedInto='' OR consolidatedInto IS NULL").get().c;
    // GE total amount
    const geRow = db.prepare("SELECT COALESCE(SUM(total),0) as t FROM apRecords WHERE type='General'").get();
    const geTotal = Math.round((geRow.t || 0) * 100) / 100;
    const recentBLs = db.prepare("SELECT * FROM jobs ORDER BY createdAt DESC LIMIT 8").all().map(r => rowToObj("jobs", r));
    // AR outstanding
    const allInvs = db.prepare("SELECT total, payments, customer, date FROM invoices WHERE (consolidatedInto='' OR consolidatedInto IS NULL)").all();
    let arOutstanding = 0; let arTotal = 0;
    const arByCust = {};
    ...
    for (const inv of allInvs) {
      ...
      arTotal = Math.round((arTotal + inv.total) * 100) / 100;
      if (bal > 0) { arOutstanding = Math.round((arOutstanding + bal) * 100) / 100; arByCust[inv.customer] = Math.round(((arByCust[inv.customer]||0) + bal)*100)/100; }
      for (const p of payments) { if ((p.date||"").startsWith(thisMonth)) monthPayments = Math.round((monthPayments + (p.amount||0))*100)/100; }
    }
    const topCust = Object.entries(arByCust).sort((a,b) => b[1]-a[1]).slice(0, 7);
    // AP outstanding (from jobs cb/ff JSON)
    let apOutstanding = 0;
    const apByPayee = {};
    const allJobsCbFf = db.prepare("SELECT cb, ff, cbEnabled, ffEnabled FROM jobs").all();
    for (const job of allJobsCbFf) {
      const proc = (chStr) => {
        if (!chStr) return;
        const ch = safeParseJSON(chStr, {});
        for (const [key, val] of Object.entries(ch)) {
          if (key.startsWith("_") || key.includes("_")) continue;
          if (!ch[key + "_disb"]) continue;
          const amt = Math.round(parseFloat(val || 0) * 100) / 100; if (amt <= 0) continue;
          const payments = ch[key + "_payments"] || [];
          const paid = payments.reduce((s, p) => s + (p.amount || 0), 0);
          const bal = Math.round((amt - paid) * 100) / 100;
          const payee = ch[key + "_payee"] || "Unknown";
          if (bal > 0) { apOutstanding = Math.round((apOutstanding + bal)*100)/100; apByPayee[payee] = Math.round(((apByPayee[payee]||0) + bal)*100)/100; }
        }
        for (const l of (ch._manualAP || [])) {
          const amt = Math.round(parseFloat(l.amount || 0) * 100) / 100; if (amt <= 0) continue;
          const paid = (l.payments || []).reduce((s, p) => s + (p.amount || 0), 0);
          const bal = Math.round((amt - paid) * 100) / 100;
          if (bal > 0) { apOutstanding = Math.round((apOutstanding + bal)*100)/100; apByPayee[l.payee||"Unknown"] = Math.round(((apByPayee[l.payee||"Unknown"]||0) + bal)*100)/100; }
        }
      };
      if (job.cbEnabled) proc(job.cb);
      if (job.ffEnabled) proc(job.ff);
    }
    const topPayee = Object.entries(apByPayee).sort((a,b) => b[1]-a[1]).slice(0, 7);
    const monthBLs = db.prepare("SELECT COUNT(*) as c FROM jobs WHERE substr(createdAt,1,7)=?").get(thisMonth).c;
    const monthInvs = db.prepare("SELECT COUNT(*) as c FROM invoices WHERE substr(date,1,7)=? AND (consolidatedInto='' OR consolidatedInto IS NULL)").get(thisMonth).c;
    return { totalBLs, openBLs, closedBLs, totalInvs, geTotal, arOutstanding, apOutstanding, arTotal, monthBLs, monthInvs, monthPayments, recentBLs, topCust, topPayee };
  });
  ...
  safeHandle("invoices:add", (_, d) => {
    const row = objToRow("invoices", { province: "", date: "", items: [], subtotal: 0, taxAmount: 0, total: 0, displayFields: {}, jobData: {}, payments: [], dueDate: "", invoicedCodes: [], fxRate: 0, invoiceCurrency: "CAD", taxLabel: "", taxRate: 0, taxComponents: [], consolidatedFrom: [], consolidatedInto: "", createdAt: new Date().toISOString(), ...d });
    db.prepare("INSERT INTO invoices (invoiceNumber,type,customer,province,date,jobId,items,subtotal,taxAmount,total,displayFields,jobData,payments,dueDate,invoicedCodes,fxRate,invoiceCurrency,taxLabel,taxRate,taxComponents,consolidatedFrom,consolidatedInto,createdAt) VALUES (@invoiceNumber,@type,@customer,@province,@date,@jobId,@items,@subtotal,@taxAmount,@total,@displayFields,@jobData,@payments,@dueDate,@invoicedCodes,@fxRate,@invoiceCurrency,@taxLabel,@taxRate,@taxComponents,@consolidatedFrom,@consolidatedInto,@createdAt)").run(row);
  ...
  safeHandle("invoices:delete", (_, num) => {
    const inv = db.prepare("SELECT * FROM invoices WHERE invoiceNumber=?").get(num);
    if (!inv) return { success: false, error: "Invoice not found" };
    const parsed = rowToObj("invoices", inv);
    const paid = (parsed.payments || []).reduce((s, p) => s + (p.amount || 0), 0);
    if (paid >= parsed.total && parsed.total > 0) return { success: false, error: "Cannot delete REMITTED invoice" };
    // Remove invoice number from all jobs that reference it
    const allJobs = db.prepare("SELECT * FROM jobs").all();
    const updateJob = db.prepare("UPDATE jobs SET cbInvoices=@cbInvoices, ffInvoices=@ffInvoices WHERE id=@id");
    const tx = db.transaction(() => {
      for (const job of allJobs) {
        const j = rowToObj("jobs", job);
        let changed = false;
        const cbInvs = (j.cbInvoices || []).filter(n => n !== num);
        const ffInvs = (j.ffInvoices || []).filter(n => n !== num);
        if (cbInvs.length !== (j.cbInvoices || []).length || ffInvs.length !== (j.ffInvoices || []).length) changed = true;
        if (changed) {
          const row = objToRow("jobs", { cbInvoices: cbInvs, ffInvoices: ffInvs });
          updateJob.run({ ...row, id: job.id });
        }
      }
      db.prepare("DELETE FROM invoices WHERE invoiceNumber=?").run(num);
    });
    tx();
    return { success: true };
  });
  ...
  // ── AP RECORDS ──
  safeHandle("ap:list", (_, { dateFrom, dateTo, payee, type, search, createdFrom, createdTo, limit, offset = 0 } = {}) => {
    ...
    if (createdFrom) { sql += " AND createdAt>=@createdFrom"; params.createdFrom = createdFrom + "T00:00:00"; }
    if (createdTo) { sql += " AND createdAt<=@createdTo"; params.createdTo = createdTo + "T23:59:59"; }
    if (search) { const q = `%${search}%`; sql += " AND (id LIKE @sq OR payee LIKE @sq OR memo LIKE @sq)"; params.sq = q; }
  ...
  safeHandle("ap:add", (_, d) => {
    const row = objToRow("apRecords", { jobId: "", customer: "", date: "", items: [], total: 0, type: "Disbursement", payments: [], dueDate: "", invoiceNum: "", fxRate: 0, memo: "", currency: "CAD", paidDate: "", taxPaid: 0, taxPaidComponents: [], createdAt: new Date().toISOString(), ...d });
    db.prepare("INSERT INTO apRecords (id,jobId,customer,payee,date,items,total,type,payments,dueDate,invoiceNum,fxRate,memo,currency,paidDate,taxPaid,taxPaidComponents,createdAt) VALUES (@id,@jobId,@customer,@payee,@date,@items,@total,@type,@payments,@dueDate,@invoiceNum,@fxRate,@memo,@currency,@paidDate,@taxPaid,@taxPaidComponents,@createdAt)").run(row);
  ...
  safeHandle("ap:count", (_, { dateFrom, dateTo, payee, type, search, createdFrom, createdTo } = {}) => {
    let where = "1=1"; const p = {};
    if (dateFrom) { where += " AND date>=@dateFrom"; p.dateFrom = dateFrom; }
    if (dateTo) { where += " AND date<=@dateTo"; p.dateTo = dateTo; }
    if (payee) { where += " AND payee=@payee"; p.payee = payee; }
    if (type) { where += " AND type=@type"; p.type = type; }
    if (createdFrom) { where += " AND createdAt>=@createdFrom"; p.createdFrom = createdFrom + "T00:00:00"; }
    if (createdTo) { where += " AND createdAt<=@createdTo"; p.createdTo = createdTo + "T23:59:59"; }
    if (search) { const q = `%${search}%`; where += " AND (id LIKE @sq OR payee LIKE @sq OR memo LIKE @sq)"; p.sq = q; }
  ...
  safeHandle("invoices:createWithJob", (_, { invoiceData, jobId, jobUpdate }) => {
    const tx = db.transaction(() => {
      // 1. Insert invoice
      const row = objToRow("invoices", { province: "", date: "", items: [], subtotal: 0, taxAmount: 0, total: 0, displayFields: {}, jobData: {}, payments: [], dueDate: "", invoicedCodes: [], fxRate: 0, invoiceCurrency: "CAD", taxLabel: "", taxRate: 0, taxComponents: [], consolidatedFrom: [], consolidatedInto: "", createdAt: new Date().toISOString(), ...invoiceData });
      db.prepare("INSERT INTO invoices (invoiceNumber,type,customer,province,date,jobId,items,subtotal,taxAmount,total,displayFields,jobData,payments,dueDate,invoicedCodes,fxRate,invoiceCurrency,taxLabel,taxRate,taxComponents,consolidatedFrom,consolidatedInto,createdAt) VALUES (@invoiceNumber,@type,@customer,@province,@date,@jobId,@items,@subtotal,@taxAmount,@total,@displayFields,@jobData,@payments,@dueDate,@invoicedCodes,@fxRate,@invoiceCurrency,@taxLabel,@taxRate,@taxComponents,@consolidatedFrom,@consolidatedInto,@createdAt)").run(row);
