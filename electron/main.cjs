...
function setupIPC() {
  ...
  safeHandle("dashboard:stats", () => {
    ...
    const totalGEs = db.prepare("SELECT COUNT(*) as c FROM apRecords WHERE type='General'").get().c;
    const recentBLs = db.prepare("SELECT * FROM jobs ORDER BY createdAt DESC LIMIT 8").all().map(r => rowToObj("jobs", r));
    const allInvs = db.prepare("SELECT total, payments, customer FROM invoices WHERE (consolidatedInto='' OR consolidatedInto IS NULL)").all();
    ...
    return { totalBLs, openBLs, closedBLs, totalInvs, totalGEs, arOutstanding, arTotal, monthBLs, monthInvs, recentBLs, topCust };
  });
