...
function PlaceholderPage({title,icon,description}){
  return(<div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"60vh",color:C.textMuted}}>
    <div style={{fontSize:48,marginBottom:16}}>{icon}</div>
    <h2 style={{fontFamily:F.display,color:C.text,margin:"0 0 8px"}}>{title}</h2>
    <p style={{fontSize:13}}>{description||"This page will be built next."}</p>
  </div>);
}
...
function blRemitStatus(job){
  ...
  if(allPaid&&!hasAccrued)return{status:"PAID",color:C.green};
...
function BLCreationPage({onOpenARAP,onOpenBLDetail,filters}){
  ...
  const closingFrom=filters.closingFrom;const setClosingFrom=filters.setClosingFrom;const[closingTo,setClosingTo]=useState("");
  ...
  // Form state
  const emptyForm={customer:"",closingDate:"",cbEnabled:false,ffEnabled:false,
    // CB fields
  ...
  const load=async()=>{
    ...
    setLoading(true);
    try{
      let cf=closingFrom,ct=closingTo;
      if(closingFrom&&!closingTo){cf=closingFrom+"-01";const[y,m]=closingFrom.split("-").map(Number);const ld=new Date(y,m,0).getDate();ct=closingFrom+"-"+String(ld).padStart(2,"0");}
      const opts={limit:PAGE_SIZE,offset:(page-1)*PAGE_SIZE,search:dSearch,closingFrom:cf,closingTo:ct};
      const[list,cnt]=await Promise.all([window.db.jobs.list(opts),window.db.jobs.count({search:dSearch,closingFrom:cf,closingTo:ct})]);
      setJobs(list||[]);setTotal(cnt||0);
    }catch(e){await showAlert("Failed to load BLs: "+e.message);}
    setLoading(false);
  };
  useEffect(()=>{load();setCustomerList([...DB.customers]);},[dSearch,closingFrom,closingTo,page]);
  ...
  const createBL=async()=>{
    ...
    try{
      ...
      const jobData={
        customer:form.customer, closingDate:form.closingDate,
  ...
  return(<div>
    ...
    {showForm&&<div style={{background:C.surface,borderRadius:10,padding:24,border:`1px solid ${C.accent}33`,marginBottom:24}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        ...
        <div><label style={S.label}>Closing Month</label><input type="month" value={form.closingDate} onChange={e=>upd("closingDate",e.target.value)} style={{fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",width:"100%"}}/></div>
      </div>
    ...
    <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"flex-end",flexWrap:"wrap"}}>
      ...
      <div><label style={S.label}>Closing Month</label><input type="month" value={closingFrom} onChange={e=>{setClosingFrom(e.target.value);setClosingTo("");setPage(1);}} style={{fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none"}}/></div>
      ...
      {(closingFrom||filterCustomer||filterStatus)&&<Button variant="ghost" size="sm" onClick={()=>{setClosingFrom("");setClosingTo("");setFilterCustomer("");setFilterStatus("");setPage(1);}}>Clear All</Button>}
    </div>
    ...
    <div style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden"}}>
      <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>
        {["ID #","BL # / MBL","Customer","Type","Closing","Pay Status","Closed",""].map(h=><th key={h} style={S.th}>{h}</th>)}
      </tr></thead>
      <tbody>{filteredJobs.length===0?<tr><td colSpan={8}><EmptyState icon="📋" text="No BL records found."/></td></tr>:filteredJobs.map(job=>{
        ...
        return(<tr key={job.id} style={{borderBottom:`1px solid ${C.border}`,opacity:isClosed?0.5:1}} onMouseEnter={e=>e.currentTarget.style.background=C.surfaceHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          ...
          <td style={S.td}><span style={{fontFamily:F.mono,fontSize:12,color:job.closingDate?C.text:C.textMuted}}>{job.closingDate||"—"}</span></td>
          ...
          <td style={S.td}><Button variant="ghost" size="sm" onClick={()=>onOpenBLDetail(job)}>Open</Button></td>
        </tr>);
      })}</tbody></table>
    </div>
...
function ARAPDetailPage({job:initialJob,onBack}){
  ...
  const[payModal,setPayModal]=useState(null); // {side:"ar"|"ap", index, code, label, amount, currency, payments:[]}
  ...
  useEffect(()=>{
    ...
    for(const[key,val]of Object.entries(charges)){
      ...
      const payments=charges[key+"_payments"]||[];
      ar.push({code:codeInfo.code,label:codeInfo.label,amount:val,currency,notes,disb,payee,payments,locked:false});
    }
  ...
  const addARLine=(codeInfo)=>{
    ...
    setArLines(p=>[...p,{code:codeInfo.code,label:codeInfo.label,amount:"",currency:"CAD",notes:"",disb:false,payee:"",payments:[],locked:false}]);setDirty(true);
  };
  // Add AP line
  const addAPLine=(codeInfo)=>{
    if(apLines.some(l=>l.code===codeInfo.code))return;
    setApLines(p=>[...p,{code:codeInfo.code,label:codeInfo.label,amount:"",currency:"CAD",notes:"",payee:"",payments:[]}]);setDirty(true);
  };
  // Remove lines
  ...
  const updAR=(i,k,v)=>{setArLines(p=>p.map((l,j)=>j===i?{...l,[k]:v}:l));if(k!=="editing")setDirty(true);};
  ...
  const saveCharges=async()=>{
    ...
    for(const l of arLines){
      ...
      if(l.disb){charges[l.code+"_disb"]=true;charges[l.code+"_payee"]=l.payee||"";}
      if(l.payments&&l.payments.length>0)charges[l.code+"_payments"]=l.payments;
    }
    // Save manual AP lines (non-disb) — include payments
    const manualAP=apLines.map(l=>({code:l.code,label:l.label,amount:l.amount,currency:l.currency,notes:l.notes,payee:l.payee,payments:l.payments||[]}));
  ...
  const invoicedCodes=useMemo(()=>{
    ...
    for(const inv of invoices){if(jobInvs.includes(inv.invoiceNumber)){(inv.invoicedCodes||[]).forEach(c=>s.add(c));}}
    return s;
  },[invoices,job,tab]);
  ...
  const generateInvoice=async(currency)=>{
    ...
    try{
      ...
      const tax=invNoTax?{rate:0,label:"No Tax",components:[]}:invCustomTaxRate?{rate:parseFloat(invCustomTaxRate)/100,label:invCustomTaxLabel||`Custom ${invCustomTaxRate}%`,components:[{type:"Custom",rate:parseFloat(invCustomTaxRate)/100,label:invCustomTaxLabel||`Custom ${invCustomTaxRate}%`}]}:provTax;
  ...
  const payStatus=(amount,payments)=>{
    ...
    const paid=(payments||[]).reduce((s,p)=>M.sum(s,p.amount),0);
    if(paid>=total)return{status:"PAID",color:C.green,paid};
    if(paid>0)return{status:"P PAID",color:C.orange,paid};
    return{status:"SLIPPED",color:C.blue,paid:0};
  };
  const openPayModal=(side,index)=>{
    const line=side==="ar"?arLines[index]:apLines[index];
    setPayModal({side,index,code:line.code,label:line.label,amount:line.amount,currency:line.currency,payments:line.payments||[]});
    setPayDate(new Date().toISOString().split("T")[0]);setPayAmt("");setPayNote("");
  };
  const addPayment=()=>{
    if(!payAmt||!payModal)return;
    const newPay={date:payDate,amount:M.round(payAmt),note:payNote};
    const updPayments=[...(payModal.payments||[]),newPay];
    if(payModal.side==="ar"){updAR(payModal.index,"payments",updPayments);}
    else{updAP(payModal.index,"payments",updPayments);}
    setPayModal(p=>({...p,payments:updPayments}));setPayAmt("");setPayNote("");
  };
  const removePayment=(pi)=>{
    if(!payModal)return;
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:20,alignItems:"flex-start"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          ...
          <div style={{display:"flex",gap:6}}>{renderCodeDropdown("ar",arDropRef,arDropdown,setArDropdown)}<Button size="sm" onClick={openInvoiceModal} disabled={availableForInvoice.length===0}>Generate Invoice</Button></div>
        </div>
        <div style={{background:C.surface,borderRadius:8,border:`1px solid ${C.border}`,padding:12}}>
          {arLines.length===0?<div style={S.empty}>No AR items. Click + Add to start.</div>:arLines.map((l,i)=>{
            ...
            const isLocked=isInvoiced&&!l.editing;
            return(<div key={l.code+i} style={{borderRadius:4,background:l.disb?`${C.orange}08`:isLocked?`${C.green}06`:"transparent",padding:l.disb?"6px 6px":"6px 0",borderBottom:i<arLines.length-1?`1px solid ${C.border}`:"none"}}>
              <div style={{display:"flex",gap:6,alignItems:"center"}}>
                ...
                {!isInvoiced&&!isLocked&&<label style={{display:"flex",alignItems:"center",gap:3,cursor:"pointer",flexShrink:0}}><input type="checkbox" checked={!!l.disb} onChange={e=>updAR(i,"disb",e.target.checked)} style={{accentColor:C.orange,width:14,height:14}}/><span style={{fontSize:9,fontWeight:600,color:l.disb?C.orange:C.textMuted,textTransform:"uppercase",letterSpacing:".3px"}}>Disb</span></label>}
                ...
                {(parseFloat(l.amount)||0)>0&&<button onClick={()=>openPayModal("ar",i)} title="Record payment" style={{background:"none",border:"none",color:C.cyan,cursor:"pointer",fontSize:12,padding:"0 2px"}}>💳</button>}
                {isInvoiced?<>{l.editing?<button onClick={()=>updAR(i,"editing",false)} style={{background:"none",border:"none",color:C.green,cursor:"pointer",fontSize:10,fontWeight:700,padding:"2px 6px",borderRadius:4,border:`1px solid ${C.green}30`}}>Done</button>:<button onClick={()=>updAR(i,"editing",true)} style={{background:"none",border:"none",color:C.textMuted,cursor:"pointer",fontSize:10,fontWeight:600,padding:"2px 6px",borderRadius:4,border:`1px solid ${C.border}`}}>Edit</button>}<Badge color={C.green}>Invoiced</Badge></>:<button onClick={()=>removeARLine(i)} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:14,fontWeight:700,padding:"0 4px"}}>×</button>}
              </div>
              {l.disb&&<div style={{display:"flex",alignItems:"center",gap:6,padding:"4px 0 0 42px"}}><span style={{fontSize:9,color:C.orange,fontWeight:600,flexShrink:0}}>→ Pay To:</span><select value={l.payee||""} onChange={e=>updAR(i,"payee",e.target.value)} style={{fontFamily:F.mono,fontSize:11,padding:"4px 6px",background:C.bg,border:`1px solid ${l.payee?C.orange+"40":C.red}`,borderRadius:5,color:C.text,outline:"none",flex:1,maxWidth:200}}><option value="">Select payee...</option>{payeeList.map(p=><option key={p.name} value={p.name}>{p.name}</option>)}</select></div>}
            </div>);
          })}
        </div>
        {/* Invoices list */}
        {invoices.filter(inv=>inv.type===tab).length>0&&<div style={{marginTop:16}}>
          <div style={{fontSize:12,fontWeight:700,color:C.text,fontFamily:F.display,marginBottom:8}}>Invoices ({tab})</div>
          {invoices.filter(inv=>inv.type===tab).map(inv=>(
            <div key={inv.invoiceNumber} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:C.card,borderRadius:6,border:`1px solid ${C.border}`,marginBottom:6}}>
              <div><span style={{fontFamily:F.mono,fontSize:12,color:C.accent,fontWeight:600}}>{inv.invoiceNumber}</span><span style={{fontSize:11,color:C.textDim,marginLeft:8}}>{inv.date}</span><span style={{fontSize:11,color:C.textDim,marginLeft:8}}>{inv.invoiceCurrency||"CAD"}</span></div>
              <div style={{display:"flex",gap:6,alignItems:"center"}}><span style={{fontFamily:F.mono,fontSize:12,color:C.text,fontWeight:600}}>{inv.invoiceCurrency||"CAD"} ${(inv.total||0).toFixed(2)}</span>
                <Button variant="ghost" size="sm" onClick={()=>setViewInvoice(inv)}>View</Button>
                <Button variant="ghost" size="sm" onClick={()=>setEditInvoice({...inv,editItems:inv.items.map(it=>({...it})),editDate:inv.date,editDueDate:inv.dueDate||"",editProvince:inv.province||"Ontario",editCurrency:inv.invoiceCurrency||"CAD",editFxRate:inv.fxRate?String(inv.fxRate):"",editNoTax:inv.taxRate===0,editCustomTaxRate:inv.taxRate!=null&&inv.taxRate!==0&&!PROVINCE_TAX[inv.province||""]?String(inv.taxRate*100):"",editCustomTaxLabel:inv.taxLabel||""})}>✏️ Edit</Button>
                <Button variant="ghost" size="sm" onClick={()=>printInvoice(inv)}>🖨</Button>
                <Button variant="ghost" size="sm" onClick={()=>saveInvoicePDF(inv)}>PDF</Button>
              </div>
            </div>
          ))}
        </div>}
        ...
        <div style={{background:C.surface,borderRadius:8,border:`1px solid ${C.border}`,padding:12}}>
          ...
          {arLines.filter(l=>l.disb).map((l,i)=>{
            ...
            return(<div key={"disb-"+l.code+i} style={{display:"flex",gap:6,alignItems:"center",padding:"6px 6px",borderBottom:`1px solid ${C.border}`,background:`${C.orange}06`,borderRadius:4,marginBottom:4}}>
              ...
              {(parseFloat(l.amount)||0)>0&&<button onClick={()=>openPayModal("ar",arLines.indexOf(l))} title="Record payment" style={{background:"none",border:"none",color:C.cyan,cursor:"pointer",fontSize:12,padding:"0 2px"}}>💳</button>}
              <Badge color={C.orange} style={{fontSize:9}}>Disb</Badge>
            </div>);
          })}
          {/* Manual AP lines */}
          {apLines.length===0&&arLines.filter(l=>l.disb).length===0?<div style={S.empty}>No AP items.</div>:apLines.map((l,i)=>(
            <div key={l.code+i} style={{display:"flex",gap:6,alignItems:"center",padding:"6px 0",borderBottom:i<apLines.length-1?`1px solid ${C.border}`:"none",flexWrap:"wrap"}}>
              <span style={{fontFamily:F.mono,fontSize:11,color:C.orange,fontWeight:600,width:36,flexShrink:0}}>{l.code}</span>
              <span style={{fontSize:11,color:C.textDim,width:100,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flexShrink:0}}>{l.label}</span>
              ...
              {(parseFloat(l.amount)||0)>0&&<button onClick={()=>openPayModal("ap",i)} title="Record payment" style={{background:"none",border:"none",color:C.cyan,cursor:"pointer",fontSize:12,padding:"0 2px"}}>💳</button>}
            </div>
          ))}
        </div>
        {/* AP records from DB */}
    ...
    {editInvoice&&<div style={S.modal} onClick={()=>setEditInvoice(null)}><div style={{background:C.surface,borderRadius:14,padding:24,width:560,maxHeight:"85vh",overflow:"auto",border:`1px solid ${C.border}`}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        ...
        <Select label="Province (Tax)" value={editInvoice.editProvince} onChange={v=>setEditInvoice(p=>({...p,editProvince:v,editCustomTaxRate:"",editCustomTaxLabel:""}))} options={PROVINCES} disabled={editInvoice.editNoTax}/>
        ...
        <div><label style={S.label}>FX Rate (USD→CAD)</label><input value={editInvoice.editFxRate||""} onChange={e=>{const v=e.target.value;if(v===""||/^\d*\.?\d*$/.test(v))setEditInvoice(p=>({...p,editFxRate:v}));}} placeholder={String(editInvoice.fxRate||"1.0000")} style={{fontFamily:F.mono,fontSize:12,padding:"7px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",width:"100%",textAlign:"right"}}/></div>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
            <label style={S.label}>Tax</label>
            <label style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer"}}><input type="checkbox" checked={!!editInvoice.editNoTax} onChange={e=>setEditInvoice(p=>({...p,editNoTax:e.target.checked,editCustomTaxRate:"",editCustomTaxLabel:""}))} style={{accentColor:C.accent}}/><span style={{fontSize:10,fontWeight:600,color:editInvoice.editNoTax?C.red:C.textMuted}}>No Tax</span></label>
          </div>
          {!editInvoice.editNoTax&&<div style={{display:"flex",gap:6,alignItems:"center"}}>
            ...
            {editInvoice.editCustomTaxRate&&<input value={editInvoice.editCustomTaxLabel||""} onChange={e=>setEditInvoice(p=>({...p,editCustomTaxLabel:e.target.value}))} placeholder="Label" style={{fontFamily:F.body,fontSize:11,padding:"5px 8px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:5,color:C.text,outline:"none",flex:1}}/>}
          </div>}
        </div>
      </div>
      <div style={{...S.label,marginBottom:8}}>Line Items</div>
      {editInvoice.editItems.map((it,i)=>(<div key={i} style={{display:"flex",gap:6,alignItems:"center",marginBottom:6}}>
        <span style={{fontFamily:F.mono,fontSize:11,color:C.accent,fontWeight:600,width:36}}>{it.code}</span>
        <span style={{fontSize:11,color:C.textDim,flex:1}}>{it.label}</span>
        <select value={it.originalCurrency||"CAD"} onChange={e=>{const items=[...editInvoice.editItems];items[i]={...items[i],originalCurrency:e.target.value};setEditInvoice(p=>({...p,editItems:items}));}} style={{fontFamily:F.mono,fontSize:11,padding:"5px 4px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:5,width:56}}><option>CAD</option><option>USD</option></select>
        <input value={it.originalAmount!=null?it.originalAmount:(it.amount||"")} onChange={e=>{const v=e.target.value;if(v===""||/^\d*\.?\d*$/.test(v)){const items=[...editInvoice.editItems];items[i]={...items[i],originalAmount:v===""?0:parseFloat(v)||0};setEditInvoice(p=>({...p,editItems:items}));}}} placeholder="0.00" style={{fontFamily:F.mono,fontSize:12,padding:"5px 8px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:5,color:C.text,outline:"none",width:90,textAlign:"right"}}/>
        <input value={it.remark||""} onChange={e=>{const items=[...editInvoice.editItems];items[i]={...items[i],remark:e.target.value};setEditInvoice(p=>({...p,editItems:items}));}} placeholder="note..." style={{fontFamily:F.body,fontSize:11,padding:"5px 8px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:5,width:100,color:C.textDim,outline:"none"}}/>
        <button onClick={()=>{const items=editInvoice.editItems.filter((_,j)=>j!==i);setEditInvoice(p=>({...p,editItems:items}));}} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:14,fontWeight:700}}>×</button>
      </div>))}
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}}>
        ...
        <Button onClick={async()=>{
          ...
          try{
            ...
            const tax=ei.editNoTax?{rate:0,label:"No Tax",components:[]}:ei.editCustomTaxRate?{rate:parseFloat(ei.editCustomTaxRate)/100,label:ei.editCustomTaxLabel||`Custom ${ei.editCustomTaxRate}%`,components:[{type:"Custom",rate:parseFloat(ei.editCustomTaxRate)/100,label:ei.editCustomTaxLabel||`Custom ${ei.editCustomTaxRate}%`}]}:provTax;
            const invCur=ei.editCurrency||"CAD";
            const newItems=ei.editItems.map(it=>{
              const origAmt=M.round(it.originalAmount);
              const origCur=it.originalCurrency||"CAD";
              const cadAmt=origCur==="USD"?M.mul(origAmt,efx):origAmt;
              return{...it,originalAmount:origAmt,originalCurrency:origCur,exchangeRate:efx,amount:cadAmt};
            });
            const subtotalCents=newItems.reduce((s,it)=>s+M.cents(it.amount),0);
            const subtotalCAD=subtotalCents/100;
            const taxComponents=(tax.components||[]).map(c=>({type:c.type,rate:c.rate,label:c.label,amount:M.mul(subtotalCAD,c.rate)}));
            const taxAmt=taxComponents.reduce((s,c)=>M.sum(s,c.amount),0);const totalCAD=M.sum(subtotalCAD,taxAmt);
            const updData={...ei,items:newItems,date:ei.editDate,dueDate:ei.editDueDate,province:ei.editProvince,invoiceCurrency:invCur,fxRate:efx,subtotal:subtotalCAD,taxAmount:taxAmt,total:totalCAD,taxLabel:tax.label,taxRate:tax.rate,taxComponents};
            delete updData.editItems;delete updData.editDate;delete updData.editDueDate;delete updData.editProvince;delete updData.editCurrency;delete updData.editFxRate;delete updData.editNoTax;delete updData.editCustomTaxRate;delete updData.editCustomTaxLabel;
            if(window.db)await window.db.invoices.update(ei.invoiceNumber,updData);
            setEditInvoice(null);await loadData();
    ...
    {/* ── Add Custom Code Modal (ARAP) ── */}
    {showAddCode&&<div style={S.modal} onClick={()=>setShowAddCode(null)}><div style={{background:C.surface,borderRadius:12,padding:24,width:400,border:`1px solid ${C.border}`}} onClick={e=>e.stopPropagation()}>
      <h3 style={{margin:"0 0 16px",fontFamily:F.display,color:C.text,fontSize:15}}>Add Custom Code ({showAddCode})</h3>
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        <Input label="Code" value={newCodeCode} onChange={v=>setNewCodeCode(v.toUpperCase())} placeholder="e.g. ISC"/>
        <Input label="Label" value={newCodeLabel} onChange={setNewCodeLabel} placeholder="e.g. Inspection Charge"/>
      </div>
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}}><Button variant="ghost" onClick={()=>setShowAddCode(null)}>Cancel</Button><Button onClick={()=>addCustomCode(showAddCode)} disabled={!newCodeCode||!newCodeLabel}>Add</Button></div>
    </div></div>}
    ...
    {showInvModal&&<div style={S.modal} onClick={()=>setShowInvModal(false)}><div style={{background:C.surface,borderRadius:14,padding:24,width:500,maxHeight:"85vh",overflow:"auto",border:`1px solid ${C.border}`}} onClick={e=>e.stopPropagation()}>
      <h3 style={{margin:"0 0 16px",fontFamily:F.display,color:C.text,fontSize:16}}>Generate {tab} Invoice</h3>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
        <Input label="Billing Date" type="date" value={invDate} onChange={setInvDate}/>
        <Input label="Due Date" type="date" value={invDueDate} onChange={setInvDueDate}/>
        <Select label="Province (Tax)" value={invProvince} onChange={v=>{setInvProvince(v);setInvCustomTaxRate("");setInvCustomTaxLabel("");}} options={PROVINCES} disabled={invNoTax}/>
        <div>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:6}}>
            <label style={S.label}>Tax</label>
            <label style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer"}}><input type="checkbox" checked={invNoTax} onChange={e=>{setInvNoTax(e.target.checked);if(e.target.checked){setInvCustomTaxRate("");setInvCustomTaxLabel("");}}} style={{accentColor:C.accent}}/><span style={{fontSize:10,fontWeight:600,color:invNoTax?C.red:C.textMuted}}>No Tax</span></label>
          </div>
          {!invNoTax&&<div style={{display:"flex",gap:6,alignItems:"center"}}>
            <input value={invCustomTaxRate} onChange={e=>{const v=e.target.value;if(v===""||/^\d*\.?\d*$/.test(v))setInvCustomTaxRate(v);}} placeholder={(PROVINCE_TAX[invProvince]||{rate:0.13}).rate*100+"%"} style={{fontFamily:F.mono,fontSize:12,padding:"7px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",width:60,textAlign:"right"}}/>
            <span style={{fontSize:10,color:C.textMuted}}>%</span>
            {invCustomTaxRate&&<input value={invCustomTaxLabel} onChange={e=>setInvCustomTaxLabel(e.target.value)} placeholder="Label e.g. GST 5%" style={{fontFamily:F.body,fontSize:11,padding:"7px 8px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",flex:1}}/>}
            {!invCustomTaxRate&&<span style={{fontSize:10,color:C.cyan,fontFamily:F.mono}}>{(PROVINCE_TAX[invProvince]||{label:"HST 13%"}).label}</span>}
            {invCustomTaxRate&&<span style={{fontSize:10,color:C.orange,fontFamily:F.mono}}>Custom</span>}
          </div>}
        </div>
        <div><label style={S.label}>FX Rate (USD→CAD)</label>
          <div style={{display:"flex",gap:6,alignItems:"center"}}>
            <input value={customFxRate} onChange={e=>{const v=e.target.value;if(v===""||/^\d*\.?\d*$/.test(v))setCustomFxRate(v);}} placeholder={fxRate?fxRate.toFixed(4):"auto"} style={{fontFamily:F.mono,fontSize:12,padding:"7px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",width:100,textAlign:"right"}}/>
            {fxRate&&!customFxRate&&<span style={{fontSize:10,color:C.cyan,fontFamily:F.mono}}>BoC {fxRate.toFixed(4)}</span>}
            {customFxRate&&parseFloat(customFxRate)!==fxRate&&<span style={{fontSize:10,color:C.orange,fontFamily:F.mono}}>Custom</span>}
          </div>
          {fxLoading&&<span style={{fontSize:10,color:C.textMuted}}>Loading BoC rate...</span>}
        </div>
      </div>
      <div style={{marginBottom:16}}><div style={{...S.label,marginBottom:8}}>Select Items</div>
        {availableForInvoice.length===0?<div style={{fontSize:12,color:C.textMuted,padding:12,textAlign:"center"}}>All items invoiced</div>:
        <div style={{display:"flex",flexDirection:"column",gap:4}}>{availableForInvoice.map(l=>{const amt=parseFloat(l.amount)||0;return(<div key={l.code} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",background:invSelCodes[l.code]?`${C.accent}0c`:"transparent",borderRadius:6,border:`1px solid ${invSelCodes[l.code]?C.accent+"30":C.border}`}}>
          <Checkbox checked={!!invSelCodes[l.code]} onChange={()=>setInvSelCodes(p=>({...p,[l.code]:!p[l.code]}))}/>
          <span style={{fontFamily:F.mono,fontSize:11,color:C.accent,fontWeight:600,width:34}}>{l.code}</span>
          <span style={{fontSize:12,color:C.text,flex:1}}>{l.label}</span>
          <span style={{fontFamily:F.mono,fontSize:12,color:C.textDim}}>{l.currency} ${amt.toFixed(2)}</span>
        </div>);})}</div>}
      </div>
      <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
        <Button variant="ghost" onClick={()=>setShowInvModal(false)}>Cancel</Button>
        <Button onClick={()=>generateInvoice("CAD")} disabled={saving||!availableForInvoice.some(l=>invSelCodes[l.code])}>🇨🇦 CAD Invoice</Button>
        <Button variant="warning" onClick={()=>generateInvoice("USD")} disabled={saving||!availableForInvoice.some(l=>invSelCodes[l.code])||(!fxRate&&!customFxRate)}>🇺🇸 USD Invoice</Button>
      </div>
    </div></div>}
    ...
    {viewInvoice&&(()=>{
      ...
      return <div style={S.modal} onClick={()=>setViewInvoice(null)}><div style={{background:C.surface,borderRadius:14,padding:28,width:640,maxHeight:"85vh",overflow:"auto",border:`1px solid ${C.border}`}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div><h2 style={{margin:0,fontFamily:F.display,color:C.text,fontSize:18}}>INVOICE</h2><span style={{fontFamily:F.mono,color:C.accent,fontSize:13}}>#{vi.invoiceNumber}</span></div>
        <div style={{display:"flex",gap:8}}><Button variant="ghost" size="sm" onClick={()=>printInvoice(vi)}>🖨 Print</Button><Button variant="ghost" size="sm" onClick={()=>saveInvoicePDF(vi)}>📄 PDF</Button><Button variant="ghost" size="sm" onClick={()=>setViewInvoice(null)}>✕</Button></div>
      </div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16,fontSize:13}}>
        <div><div style={{...S.label,marginBottom:2}}>Bill To</div><div style={{color:C.text,fontWeight:600}}>{vi.customer}</div></div>
        <div><div style={{...S.label,marginBottom:2}}>Date</div><div style={{color:C.text,fontFamily:F.mono}}>{vi.date}</div></div>
        <div><div style={{...S.label,marginBottom:2}}>Currency</div><Badge>{viCur}</Badge></div>
        {viFx>0&&viFx!==1&&<div><div style={{...S.label,marginBottom:2}}>FX Rate</div><div style={{color:C.cyan,fontFamily:F.mono}}>1 USD = {viFx.toFixed(4)} CAD</div></div>}
      </div>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}><thead><tr style={{borderBottom:`2px solid ${C.border}`}}><th style={{textAlign:"left",padding:"8px 0",color:C.textMuted,fontSize:10,textTransform:"uppercase"}}>Item</th><th style={{textAlign:"right",padding:"8px 0",color:C.textMuted,fontSize:10,textTransform:"uppercase"}}>Original</th><th style={{textAlign:"right",padding:"8px 0",color:C.textMuted,fontSize:10,textTransform:"uppercase"}}>Amount ({viCur})</th></tr></thead>
      <tbody>{vi.items.map((it,i)=>{
        const origAmt=it.originalAmount||it.amount||0;const origCur=it.originalCurrency||"CAD";
        let dispAmt;if(viCur==="USD"){dispAmt=origCur==="USD"?origAmt:M.mul(origAmt,1/viFx);}else{dispAmt=origCur==="USD"?M.mul(origAmt,viFx):origAmt;}
        return <tr key={i} style={{borderBottom:`1px solid ${C.border}`}}>
          <td style={{padding:"9px 0",color:C.text}}>{it.label}{it.remark&&<div style={{fontSize:10,color:C.textMuted}}>{it.remark}</div>}
            {origCur!==viCur&&viFx>0&&<div style={{fontSize:10,color:C.cyan}}>{origCur==="USD"?`USD $${origAmt.toFixed(2)} × ${viFx.toFixed(4)} = CAD $${M.mul(origAmt,viFx).toFixed(2)}`:`CAD $${origAmt.toFixed(2)} ÷ ${viFx.toFixed(4)} = USD $${M.mul(origAmt,1/viFx).toFixed(2)}`}</div>}
          </td>
          <td style={{padding:"9px 0",textAlign:"right",fontFamily:F.mono,fontSize:11,color:C.textDim}}>{origCur} ${origAmt.toFixed(2)}</td>
          <td style={{padding:"9px 0",textAlign:"right",fontFamily:F.mono,fontWeight:600}}>{viCur} ${dispAmt.toFixed(2)}</td>
        </tr>;
      })}</tbody></table>
      <div style={{marginTop:14,borderTop:`1px solid ${C.border}`,paddingTop:10}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:C.textDim}}>Subtotal</span><span style={{fontFamily:F.mono}}>CAD ${vi.subtotal.toFixed(2)}</span></div>
        {(vi.taxComponents||[]).length>0?(vi.taxComponents||[]).filter(c=>(c.amount||0)>0).map((c,ci)=><div key={ci} style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:C.textDim}}>{c.label}</span><span style={{fontFamily:F.mono}}>CAD ${(c.amount||0).toFixed(2)}</span></div>):(vi.taxAmount>0&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:4}}><span style={{color:C.textDim}}>{vi.taxLabel||"Tax"}</span><span style={{fontFamily:F.mono}}>CAD ${vi.taxAmount.toFixed(2)}</span></div>)}
        <div style={{display:"flex",justifyContent:"space-between",marginTop:8,paddingTop:8,borderTop:`2px solid ${C.accent}`}}><span style={{fontWeight:700,fontSize:15}}>Total (CAD)</span><span style={{color:C.accent,fontWeight:700,fontFamily:F.mono,fontSize:15}}>CAD ${vi.total.toFixed(2)}</span></div>
        {viCur==="USD"&&viFx>0&&<div style={{display:"flex",justifyContent:"space-between",marginTop:4}}><span style={{fontWeight:600,fontSize:13,color:C.textDim}}>Total (USD)</span><span style={{color:C.cyan,fontWeight:700,fontFamily:F.mono,fontSize:13}}>USD ${M.mul(vi.total,1/viFx).toFixed(2)}</span></div>}
      </div>
    </div></div>;
    })()}
    ...
    {payModal&&<div style={S.modal} onClick={()=>setPayModal(null)}><div style={{background:C.surface,borderRadius:14,padding:24,width:480,maxHeight:"85vh",overflow:"auto",border:`1px solid ${C.border}`}} onClick={e=>e.stopPropagation()}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
        <div><h3 style={{margin:0,fontFamily:F.display,color:C.text,fontSize:15}}>{payModal.side==="ar"?"Payment Received":"Payment Made"}</h3><span style={{fontSize:12,color:C.textDim}}>{payModal.code} — {payModal.label}</span></div>
        <Button variant="ghost" size="sm" onClick={()=>setPayModal(null)}>✕</Button>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,padding:"10px 14px",background:C.card,borderRadius:8,border:`1px solid ${C.border}`}}>
        <div><div style={{fontSize:10,color:C.textMuted,textTransform:"uppercase"}}>Total</div><div style={{fontFamily:F.mono,fontSize:16,color:C.text,fontWeight:700}}>{payModal.currency} ${M.round(payModal.amount).toFixed(2)}</div></div>
        <div><div style={{fontSize:10,color:C.textMuted,textTransform:"uppercase"}}>{payModal.side==="ar"?"Received":"Paid"}</div><div style={{fontFamily:F.mono,fontSize:16,color:C.green,fontWeight:700}}>{payModal.currency} ${(payModal.payments||[]).reduce((s,p)=>M.sum(s,p.amount),0).toFixed(2)}</div></div>
        <div><div style={{fontSize:10,color:C.textMuted,textTransform:"uppercase"}}>Remaining</div><div style={{fontFamily:F.mono,fontSize:16,color:C.red,fontWeight:700}}>{payModal.currency} ${M.sub(payModal.amount,(payModal.payments||[]).reduce((s,p)=>M.sum(s,p.amount),0)).toFixed(2)}</div></div>
      </div>
      {(payModal.payments||[]).length>0&&<div style={{marginBottom:16}}>
        <div style={{...S.label,marginBottom:6}}>Payment History</div>
        {payModal.payments.map((p,pi)=>(
          <div key={pi} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 10px",background:C.bg,borderRadius:6,marginBottom:4,border:`1px solid ${C.border}`}}>
            <div style={{display:"flex",gap:10,alignItems:"center"}}><span style={{fontFamily:F.mono,fontSize:11,color:C.textDim}}>{p.date}</span><span style={{fontFamily:F.mono,fontSize:12,color:C.green,fontWeight:600}}>{payModal.currency} ${M.round(p.amount).toFixed(2)}</span>{p.note&&<span style={{fontSize:10,color:C.textDim}}>{p.note}</span>}</div>
            <button onClick={()=>removePayment(pi)} style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:13,fontWeight:700}}>×</button>
          </div>
        ))}
      </div>}
      <div style={{...S.label,marginBottom:6}}>Add Payment</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
        <Input label="Date" type="date" value={payDate} onChange={setPayDate}/>
        <Input label="Amount" value={payAmt} onChange={v=>{if(v===""||/^\d*\.?\d*$/.test(v))setPayAmt(v);}} placeholder="0.00" inputMode="numeric"/>
      </div>
      <Input label="Note (optional)" value={payNote} onChange={setPayNote} placeholder="Check #, reference..." style={{marginBottom:12}}/>
      <div style={{display:"flex",justifyContent:"flex-end",gap:8}}>
        <Button variant="ghost" onClick={()=>setPayModal(null)}>Close</Button>
        <Button onClick={addPayment} disabled={!payAmt||!payDate}>+ Add Payment</Button>
      </div>
    </div></div>}
  </div>);
}
...
function BLDetailPage({job:initialJob,onBack}){
  ...
  useEffect(()=>{
    const j=job;
    setForm({customer:j.customer||"",closingDate:j.closingDate||"",cbEnabled:!!j.cbEnabled,ffEnabled:!!j.ffEnabled,
  ...
  const save=async()=>{
    ...
    try{
      ...
      const sizeStr=ffContainers.map(c=>c.size).join(", ");
      const sealStr=ffContainers.map(c=>c.seal||"").join(", ");
      const d={customer:form.customer,closingDate:form.closingDate,cbEnabled:form.cbEnabled,ffEnabled:form.ffEnabled,
...
// ═══════ AR/AP SEARCH PAGE (main arap tab) ═══════
function ARAPSearchPage({onOpenARAP}){
  const[jobs,setJobs]=useState([]);const[search,setSearch]=useState("");const dSearch=useDebounce(search);
  const[closingMonth,setClosingMonth]=useState("");
  const[filterCustomer,setFilterCustomer]=useState("");const[filterStatus,setFilterStatus]=useState("");
  const[page,setPage]=useState(1);const[total,setTotal]=useState(0);const PAGE_SIZE=50;
  const[loading,setLoading]=useState(true);const[customerList,setCustomerList]=useState([]);

  const load=async()=>{
    if(!window.db)return;
    setLoading(true);
    try{
      let closingFrom="",closingTo="";
      if(closingMonth){
        closingFrom=closingMonth+"-01";
        const[y,m]=closingMonth.split("-").map(Number);
        const lastDay=new Date(y,m,0).getDate();
        closingTo=closingMonth+"-"+String(lastDay).padStart(2,"0");
      }
      const opts={limit:PAGE_SIZE,offset:(page-1)*PAGE_SIZE,search:dSearch,closingFrom,closingTo};
      const[list,cnt]=await Promise.all([window.db.jobs.list(opts),window.db.jobs.count({search:dSearch,closingFrom,closingTo})]);
      setJobs(list||[]);setTotal(cnt||0);
    }catch(e){await showAlert("Failed to load: "+e.message);}
    setLoading(false);
  };
  useEffect(()=>{load();setCustomerList([...DB.customers]);},[dSearch,closingMonth,page]);
  ...
  return(<div>
    <h1 style={S.pt}>💰 AR / AP</h1>

    {/* Search & Filters */}
    <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"flex-end",flexWrap:"wrap"}}>
      <Input value={search} onChange={v=>{setSearch(v);setPage(1);}} placeholder="Search ID, BL, MBL, Customer..." style={{flex:1,minWidth:180}}/>
      <div><label style={S.label}>Closing Month</label><input type="month" value={closingMonth} onChange={e=>{setClosingMonth(e.target.value);setPage(1);}} style={{fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none"}}/></div>
      <div><label style={S.label}>Customer</label><select value={filterCustomer} onChange={e=>setFilterCustomer(e.target.value)} style={{fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",minWidth:120}}><option value="">All</option>{customerList.map(c=><option key={c.id} value={c.name}>{c.name}</option>)}</select></div>
      <div><label style={S.label}>Status</label><select value={filterStatus} onChange={e=>setFilterStatus(e.target.value)} style={{fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",minWidth:100}}><option value="">All</option><option value="ACCRUED">ACCRUED</option><option value="SLIPPED">SLIPPED</option><option value="PAID">PAID</option></select></div>
      {(closingMonth||filterCustomer||filterStatus)&&<Button variant="ghost" size="sm" onClick={()=>{setClosingMonth("");setFilterCustomer("");setFilterStatus("");setPage(1);}}>Clear All</Button>}
    </div>
    ...
    <div style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden"}}>
      ...
      <tbody>{filteredJobs.length===0?<tr><td colSpan={7}><EmptyState text="No BL records found."/></td></tr>:filteredJobs.map(job=>{
        const rs=blRemitStatus(job);
        const isClosed=!!job.closedMonth;
        return(<tr key={job.id} style={{borderBottom:`1px solid ${C.border}`,opacity:isClosed?0.5:1}} onMouseEnter={e=>e.currentTarget.style.background=C.surfaceHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          <td style={S.td}><span onClick={()=>onOpenARAP(job)} style={{fontFamily:F.mono,fontSize:12,color:C.accent,fontWeight:600,cursor:"pointer",textDecoration:"underline",textDecorationColor:C.accent+"40"}} title="Open AR/AP">{job.id}</span></td>
          <td style={S.td}><span style={{fontFamily:F.mono,fontSize:12,color:C.text}}>{job.bl||job.mbl||"—"}</span>{job.bl&&job.mbl&&<span style={{fontSize:10,color:C.textMuted,marginLeft:6}}>MBL: {job.mbl}</span>}</td>
          <td style={S.td}><span style={{fontSize:13,color:C.text}}>{job.customer}</span></td>
          <td style={S.td}><div style={{display:"flex",gap:4}}>
            {job.cbEnabled&&<Badge color={C.accent}>CB</Badge>}
            {job.ffEnabled&&<Badge color={C.orange}>FF</Badge>}
...
function GeneralExpensePage(){
  ...
  const allCodes=[...CB_CODES,...FF_CODES,...customCodes];

  // Form state
  const emptyForm={code:"",label:"",amount:"",currency:"CAD",notes:"",payee:"",closingMonth:"",registeredDate:new Date().toISOString().split("T")[0],paidDate:"",memo:""};
  const[form,setForm]=useState(emptyForm);
  const upd=(k,v)=>setForm(p=>({...p,[k]:v}));
  ...
  const load=async()=>{
    ...
    try{
      ...
      if(closingMonth){filtered=filtered.filter(a=>(a.memo||"").includes("closing:"+closingMonth));}
  ...
  useEffect(()=>{load();},[dSearch,closingMonth,page]);

  const totalPages=Math.max(1,Math.ceil(total/PAGE_SIZE));

  const saveExpense=async()=>{
    if(!form.code||!form.amount){await showAlert("Please select a code and enter an amount.");return;}
    setSaving(true);
    try{
      if(editId){
        // Update existing
        if(window.db)await window.db.ap.update(editId,{
          payee:form.payee,date:form.registeredDate,
          items:[{code:form.code,label:form.label,amount:parseFloat(form.amount)||0}],
          total:parseFloat(form.amount)||0,
          memo:"closing:"+form.closingMonth+"|"+form.notes,
          currency:form.currency,paidDate:form.paidDate
        });
        setEditId(null);
      } else {
        const id=await dbNextGeneralId();
        await dbAddAP({id,jobId:"",customer:"",payee:form.payee,date:form.registeredDate,items:[{code:form.code,label:form.label,amount:parseFloat(form.amount)||0}],total:parseFloat(form.amount)||0,type:"General",payments:[],dueDate:"",invoiceNum:"",fxRate:0,memo:"closing:"+form.closingMonth+"|"+form.notes,currency:form.currency,paidDate:form.paidDate});
      }
  ...
  const startEdit=(item)=>{
    ...
    const code=(item.items||[])[0]?.code||"";const label=(item.items||[])[0]?.label||"";
    setForm({code,label,amount:String(item.total||""),currency:item.currency||"CAD",notes:parsed.notes,payee:item.payee||"",closingMonth:parsed.closingMonth,registeredDate:item.date||"",paidDate:item.paidDate||"",memo:""});
  ...
  const parseMemo=(memo)=>{
    ...
    const notes=(memo||"").split("|").slice(1).join("|");
    return{closingMonth:closing?closing[1]:"",notes};
  };
  ...
  return(<div>
    ...
    {showForm&&<div style={{background:C.surface,borderRadius:10,padding:24,border:`1px solid ${C.accent}33`,marginBottom:24}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:12}}>
        {/* Code selector */}
        <div ref={codeDropRef} style={{position:"relative"}}>
          <label style={S.label}>Code</label>
          <div onClick={()=>setCodeDropdown(!codeDropdown)} style={{fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:form.code?C.text:C.textMuted,cursor:"pointer"}}>{form.code?`${form.code} — ${form.label}`:"Select code..."}</div>
          {codeDropdown&&<div style={{position:"absolute",top:"100%",left:0,background:C.surface,border:`1px solid ${C.border}`,borderRadius:8,boxShadow:"0 8px 24px rgba(0,0,0,0.12)",zIndex:100,minWidth:260,maxHeight:280,overflow:"auto",marginTop:4}}>
            {allCodes.map(c=><div key={c.code} onClick={()=>{upd("code",c.code);upd("label",c.label);setCodeDropdown(false);}} style={{padding:"8px 12px",cursor:"pointer",fontSize:12,display:"flex",gap:8}} onMouseEnter={e=>e.currentTarget.style.background=C.surfaceHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}><span style={{fontFamily:F.mono,color:C.accent,fontWeight:600,fontSize:11}}>{c.code}</span><span style={{color:C.text}}>{c.label}</span></div>)}
            <div style={{borderTop:`1px solid ${C.border}`,padding:"8px 12px"}}><button onClick={()=>{setShowAddCode(true);setCodeDropdown(false);}} style={{background:"none",border:"none",color:C.green,cursor:"pointer",fontSize:11,fontWeight:600,fontFamily:F.body}}>+ New Custom Code</button></div>
          </div>}
        </div>
        <Input label="Amount" value={form.amount} onChange={v=>{if(v===""||/^\d*\.?\d*$/.test(v))upd("amount",v);}} placeholder="0.00" inputMode="numeric"/>
        ...
        <div><label style={S.label}>Closing Month</label><input type="month" value={form.closingMonth} onChange={e=>upd("closingMonth",e.target.value)} style={{width:"100%",fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none"}}/></div>
        ...
        <Input label="Notes" value={form.notes} onChange={v=>upd("notes",v)} placeholder="Description..." style={{gridColumn:"span 2"}}/>
      </div>
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}}>
        <Button variant="ghost" onClick={()=>{setShowForm(false);setForm(emptyForm);setEditId(null);}}>Cancel</Button>
        <Button onClick={saveExpense} disabled={saving||!form.code||!form.amount}>{saving?"Saving...":(editId?"💾 Update":"Save Expense")}</Button>
      </div>
    </div>}
    ...
    <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"flex-end",flexWrap:"wrap"}}>
      ...
      <div><label style={S.label}>Closing Month</label><input type="month" value={closingMonth} onChange={e=>{setClosingMonth(e.target.value);setPage(1);}} style={{fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none"}}/></div>
      {closingMonth&&<Button variant="ghost" size="sm" onClick={()=>{setClosingMonth("");setPage(1);}}>Clear</Button>}
    </div>

    {/* ── List ── */}
    <div style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden"}}>
      <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>
        {["ID","Code","Payee","Amount","Status","Closing","Registered","Paid Date","Notes",""].map(h=><th key={h} style={S.th}>{h}</th>)}
      </tr></thead>
      <tbody>{items.length===0?<tr><td colSpan={10} style={S.empty}>No general expenses.</td></tr>:items.map(item=>{
        const parsed=parseMemo(item.memo);
        const code=(item.items||[])[0]?.code||"";const label=(item.items||[])[0]?.label||"";
        const amt=M.round(item.total);
        const hasPaidDate=!!item.paidDate;
        const statusLabel=amt<=0?"ACCRUED":hasPaidDate?"PAID":"SLIPPED";
        ...
        return(<tr key={item.id} style={{borderBottom:`1px solid ${C.border}`,cursor:"pointer"}} onClick={()=>startEdit(item)} onMouseEnter={e=>e.currentTarget.style.background=C.surfaceHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          <td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.accent,fontWeight:600}}>{item.id}</span></td>
          <td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.orange}}>{code}</span>{label&&<span style={{fontSize:10,color:C.textDim,marginLeft:4}}>{label}</span>}</td>
          ...
          <td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.textDim}}>{parsed.closingMonth||"—"}</span></td>
...
function ClosingPage(){
  ...
  const[closingMonth,setClosingMonth]=useState("");const[search,setSearch]=useState("");const dSearch=useDebounce(search);
  ...
  const load=async()=>{
    ...
    // Load BLs
    let cf="",ct="";
    if(closingMonth){cf=closingMonth+"-01";const[y,m]=closingMonth.split("-").map(Number);const ld=new Date(y,m,0).getDate();ct=closingMonth+"-"+String(ld).padStart(2,"0");}
    const opts={limit:999,offset:0,search:dSearch,closingFrom:cf,closingTo:ct};
    const jList=await window.db.jobs.list(opts);
    setJobs(jList||[]);
    // Load GEs
    const allAp=await window.db.ap.list({dateFrom:"",dateTo:"",payee:"",limit:999,offset:0});
    ...
    if(closingMonth){ges=ges.filter(a=>(a.memo||"").includes("closing:"+closingMonth));}
    setGeItems(ges);
    setSelected(new Set());
  };
  useEffect(()=>{load();},[dSearch,closingMonth]);

  // Combine and paginate
  const allRows=[];
  jobs.forEach(j=>allRows.push({type:"BL",id:j.id,data:j,closingMonth:j.closingDate||"",closed:!!j.closedMonth,closedMonth:j.closedMonth||""}));
  geItems.forEach(g=>{const pm=parseMemo(g.memo);allRows.push({type:"GE",id:g.id,data:g,closingMonth:pm.closingMonth,closed:!!g.closedMonth,closedMonth:g.closedMonth||""});});
  ...
  const handleClose=async()=>{
    ...
    if(!await showConfirm(`Close ${selected.size} item(s) for month ${month}?`))return;
    setSaving(true);
    try{
      const jobIds=[],geIds=[];
      for(const key of selected){const[t,id]=key.split(":");if(t==="BL")jobIds.push(id);else geIds.push(id);}
      if(jobIds.length>0)await window.db.jobs.bulkClose(jobIds,month);
      if(geIds.length>0)await window.db.ap.bulkClose(geIds,month);
      await load();
    }catch(e){await showAlert("Error: "+e.message);}
    setSaving(false);
  };
  ...
  return(<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
      ...
      <Button onClick={handleClose} disabled={saving||selected.size===0}>{saving?"Closing...":(`🔒 Close (${selected.size})`)}</Button>
    </div>
    ...
    <div style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,overflow:"hidden"}}>
      ...
      <tbody>{pageRows.length===0?<tr><td colSpan={9} style={S.empty}>No records found.</td></tr>:pageRows.map(row=>{
        ...
        const rs=row.type==="BL"?blRemitStatus(row.data):(()=>{const amt=M.round(row.data.total);if(amt<=0)return{status:"ACCRUED",color:C.purple};return row.data.paidDate?{status:"PAID",color:C.green}:{status:"SLIPPED",color:C.blue};})();
        return(<React.Fragment key={key}>
          <tr style={{borderBottom:`1px solid ${C.border}`,opacity:isClosed?0.45:1}} onMouseEnter={e=>{if(!isClosed)e.currentTarget.style.background=C.surfaceHover;}} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
            ...
            <td style={S.td}><span style={{fontFamily:F.mono,fontSize:12,color:C.accent,fontWeight:600}}>{row.id}</span></td>
            <td style={S.td}><span style={{fontSize:12,color:C.text}}>{row.type==="BL"?row.data.customer:(row.data.payee||"—")}</span></td>
            <td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.textDim}}>{row.closingMonth||"—"}</span></td>
            <td style={S.td}>{rs.status&&<span style={{fontSize:10,fontWeight:700,color:rs.color,padding:"2px 8px",borderRadius:4,border:`1px solid ${rs.color}30`,background:`${rs.color}10`}}>{rs.status}</span>}</td>
          ...
          {isExpanded&&<tr style={{background:`${C.accent}04`}}><td colSpan={9} style={{padding:"8px 16px 12px 52px"}}>
            {row.type==="BL"?(()=>{
              ...
              const code=(row.data.items||[])[0]?.code||"";const label=(row.data.items||[])[0]?.label||"";
              return(<div style={{display:"flex",flexDirection:"column",gap:4,fontSize:11}}>
                <div style={{display:"flex",gap:12}}>
                  <span><strong>Code:</strong> <span style={{fontFamily:F.mono,color:C.orange}}>{code}</span> {label}</span>
                  <span><strong>Amount:</strong> <span style={{fontFamily:F.mono}}>{row.data.currency||"CAD"} ${(row.data.total||0).toFixed(2)}</span></span>
                </div>
                <div style={{display:"flex",gap:12}}>
...
function ReportsPage(){
  ...
  const genClosing=async()=>{
    ...
    try{
      const cf=clMonth+"-01";const[y,m]=clMonth.split("-").map(Number);const ld=new Date(y,m,0).getDate();const ct=clMonth+"-"+String(ld).padStart(2,"0");
      const allJobs=await window.db.jobs.list({limit:9999,offset:0,search:"",closingFrom:cf,closingTo:ct});
      ...
      for(const ge of closedGEs){const amt=M.round(ge.total);totalGE=M.sum(totalGE,amt);geRows.push({id:ge.id,payee:ge.payee||"—",code:(ge.items||[])[0]?.code||"",amount:amt,currency:ge.currency||"CAD"});}
      ...
      for(const ge of accruedGEs){const amt=M.round(ge.total);accGET=M.sum(accGET,amt);accGERows.push({id:ge.id,payee:ge.payee||"—",code:(ge.items||[])[0]?.code||"",amount:amt,currency:ge.currency||"CAD"});}
      // Tax
      ...
      const taxRows=Object.entries(taxMap).filter(([,v])=>v.collected>0||v.paid>0).map(([l,v])=>({label:l,type:v.type,collected:v.collected,paid:v.paid,net:v.collected-v.paid})).sort((a,b)=>(txOrd[a.type]??9)-(txOrd[b.type]??9));
...
${aGH?`<h3>Accrued GE (${r.accruedGECount})</h3><table><thead><tr><th>ID</th><th>Code</th><th>Payee</th><th style="text-align:right">Amount</th></tr></thead><tbody>${aGH}<tr style="border-top:2px solid #7c6b8a;font-weight:700;color:#7c6b8a"><td colspan="3">Subtotal</td><td style="text-align:right">${fmt(r.accruedGETotal)}</td></tr></tbody></table>`:""}
  ...
  const genSOA=async()=>{
    ...
    try{
      const allInv=await window.db.invoices.list({limit:9999,offset:0});
      const custInvs=(allInv||[]).filter(i=>i.customer===soaCust).sort((a,b)=>(a.date||"").localeCompare(b.date||""));
      const rows=custInvs.map(inv=>{
        const paid=(inv.payments||[]).reduce((s,p)=>M.sum(s,p.amount),0);
        return{invNum:inv.invoiceNumber,date:inv.date,dueDate:inv.dueDate||"",total:M.round(inv.total),currency:inv.invoiceCurrency||"CAD",fxRate:inv.fxRate||1,paid,balance:M.sub(inv.total,paid)};
      });
      const totalOwed=rows.reduce((s,r)=>M.sum(s,r.balance),0);const totalInvoiced=rows.reduce((s,r)=>M.sum(s,r.total),0);const totalPaid=rows.reduce((s,r)=>M.sum(s,r.paid),0);
  ...
  const printSOA=()=>{
    ...
    const rowsH=d.rows.map(r=>`<tr><td>${esc(r.invNum)}</td><td>${esc(r.date)}</td><td>${esc(r.dueDate)}</td><td style="text-align:right">${fmt(r.total)}</td><td style="text-align:right">${fmt(r.paid)}</td><td style="text-align:right;font-weight:600;color:${r.balance>0?"#c4533a":"#3a8a5c"}">${fmt(r.balance)}</td></tr>`).join("");
...
<table><thead><tr><th>Invoice #</th><th>Date</th><th>Due Date</th><th style="text-align:right">Amount</th><th style="text-align:right">Paid</th><th style="text-align:right">Balance</th></tr></thead><tbody>${rowsH}<tr class="total"><td colspan="3">Total</td><td style="text-align:right">${fmt(d.totalInvoiced)}</td><td style="text-align:right">${fmt(d.totalPaid)}</td><td style="text-align:right;color:#c4533a">${fmt(d.totalOwed)}</td></tr></tbody></table>
...
${co.notesTerms?`<div style="margin-top:30px;padding-top:16px;border-top:1px solid #ccc"><div style="font-size:11px;font-weight:700;color:#555;margin-bottom:4px">Terms</div><div style="font-size:11px;color:#444;white-space:pre-wrap">${esc(co.notesTerms)}</div></div>`:""}
  ...
  const genApAging=async()=>{
    ...
    try{
      const allJobs=await window.db.jobs.list({limit:9999,offset:0,search:"",closingFrom:"",closingTo:""});
  ...
  const genRevByCust=async()=>{
    ...
    try{
      ...
      const custMap={};
      for(const job of (allJobs||[])){
        if(revFrom&&(job.closingDate||"")<revFrom)continue;if(revTo&&(job.closingDate||"")>revTo)continue;
        let ar=0,ap=0;
  ...
  const genProfitByPeriod=async()=>{
    ...
    try{
      ...
      const monthMap={};
      for(const job of (allJobs||[])){
        const cm=job.closingDate?.slice(0,7)||"Unknown";
  ...
  return(<div>
    ...
    {tab==="closing"&&<div>
      ...
      {clReport&&<div>
        ...
        {clReport.closedBLs.length>0&&<div style={{marginBottom:20}}><div style={{fontSize:14,fontWeight:700,color:C.text,fontFamily:F.display,marginBottom:10}}>Closed BLs ({clReport.closedBLCount})</div>
          <div style={{background:C.surface,borderRadius:8,border:`1px solid ${C.border}`,overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>{["Job ID","Customer","BL #","AR","AP","Net"].map(h=><th key={h} style={{...S.th,textAlign:["AR","AP","Net"].includes(h)?"right":"left"}}>{h}</th>)}</tr></thead><tbody>
            {clReport.closedBLs.map(b=><tr key={b.id} style={{borderBottom:`1px solid ${C.border}`}}><td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.accent,fontWeight:600}}>{b.id}</span></td><td style={S.td}>{b.customer}</td><td style={S.td}><span style={{fontFamily:F.mono,fontSize:11}}>{b.bl}</span></td><td style={{...S.td,textAlign:"right"}}><span style={{fontFamily:F.mono,color:C.green}}>{fmt(b.ar)}</span></td><td style={{...S.td,textAlign:"right"}}><span style={{fontFamily:F.mono,color:C.red}}>{fmt(b.ap)}</span></td><td style={{...S.td,textAlign:"right"}}><span style={{fontFamily:F.mono,fontWeight:700,color:b.net>=0?C.green:C.red}}>{fmt(b.net)}</span></td></tr>)}
        ...
        {clReport.closedGEs.length>0&&<div style={{marginBottom:20}}><div style={{fontSize:14,fontWeight:700,color:C.text,fontFamily:F.display,marginBottom:10}}>Closed GE ({clReport.closedGECount})</div>
          <div style={{background:C.surface,borderRadius:8,border:`1px solid ${C.border}`,overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>{["ID","Code","Payee","Amount"].map(h=><th key={h} style={{...S.th,textAlign:h==="Amount"?"right":"left"}}>{h}</th>)}</tr></thead><tbody>
            {clReport.closedGEs.map(g=><tr key={g.id} style={{borderBottom:`1px solid ${C.border}`}}><td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.orange,fontWeight:600}}>{g.id}</span></td><td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.orange}}>{g.code}</span></td><td style={S.td}>{g.payee}</td><td style={{...S.td,textAlign:"right"}}><span style={{fontFamily:F.mono}}>{fmt(g.amount)}</span></td></tr>)}
        ...
        {clReport.taxRows.length>0&&<div style={{marginBottom:20}}><div style={{fontSize:14,fontWeight:700,color:C.text,fontFamily:F.display,marginBottom:10}}>Tax Summary</div>
          <div style={{background:C.surface,borderRadius:8,border:`1px solid ${C.border}`,overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>{["Tax Type","Collected (AR)","Paid (AP)","Net"].map(h=><th key={h} style={{...S.th,textAlign:h==="Tax Type"?"left":"right"}}>{h}</th>)}</tr></thead><tbody>
        ...
        {(clReport.accruedBLs.length>0||clReport.accruedGEs.length>0)&&<div style={{marginBottom:20}}>
          ...
          {clReport.accruedBLs.length>0&&<div style={{background:C.surface,borderRadius:8,border:`1px solid ${C.purple}30`,overflow:"hidden",marginBottom:12}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:`${C.purple}08`}}>{["Job ID","Customer","BL #","AR","AP","Net"].map(h=><th key={h} style={{...S.th,textAlign:["AR","AP","Net"].includes(h)?"right":"left"}}>{h}</th>)}</tr></thead><tbody>
            {clReport.accruedBLs.map(b=><tr key={b.id} style={{borderBottom:`1px solid ${C.border}`}}><td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.purple,fontWeight:600}}>{b.id}</span></td><td style={S.td}>{b.customer}</td><td style={S.td}><span style={{fontFamily:F.mono,fontSize:11}}>{b.bl}</span></td><td style={{...S.td,textAlign:"right"}}><span style={{fontFamily:F.mono}}>{fmt(b.ar)}</span></td><td style={{...S.td,textAlign:"right"}}><span style={{fontFamily:F.mono}}>{fmt(b.ap)}</span></td><td style={{...S.td,textAlign:"right"}}><span style={{fontFamily:F.mono,fontWeight:600}}>{fmt(b.net)}</span></td></tr>)}
          </tbody></table></div>}
          {clReport.accruedGEs.length>0&&<div style={{background:C.surface,borderRadius:8,border:`1px solid ${C.purple}30`,overflow:"hidden"}}><table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:`${C.purple}08`}}>{["ID","Code","Payee","Amount"].map(h=><th key={h} style={{...S.th,textAlign:h==="Amount"?"right":"left"}}>{h}</th>)}</tr></thead><tbody>
            {clReport.accruedGEs.map(g=><tr key={g.id} style={{borderBottom:`1px solid ${C.border}`}}><td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.purple,fontWeight:600}}>{g.id}</span></td><td style={S.td}><span style={{fontFamily:F.mono,fontSize:11}}>{g.code}</span></td><td style={S.td}>{g.payee}</td><td style={{...S.td,textAlign:"right"}}><span style={{fontFamily:F.mono}}>{fmt(g.amount)}</span></td></tr>)}
          </tbody></table></div>}
        </div>}
      </div>}
    </div>}
    ...
    {tab==="soa"&&<div>
      ...
      {soaData&&<div>
        ...
        <div style={{background:C.surface,borderRadius:8,border:`1px solid ${C.border}`,overflow:"hidden"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>
            {["Invoice #","Date","Due Date","Amount","Paid","Balance"].map(h=><th key={h} style={{...S.th,textAlign:["Amount","Paid","Balance"].includes(h)?"right":"left"}}>{h}</th>)}
          </tr></thead><tbody>
            {soaData.rows.length===0?<tr><td colSpan={6}><EmptyState text="No invoices found for this customer."/></td></tr>:soaData.rows.map(r=><tr key={r.invNum} style={{borderBottom:`1px solid ${C.border}`}}>
              <td style={S.td}><span style={{fontFamily:F.mono,fontSize:12,color:C.accent,fontWeight:600}}>{r.invNum}</span></td>
...
const NAV=[
  ...
  {id:"stl",label:"STL (Settlement)",icon:"📑"},
...
function AppInner(){
  const[page,setPage]=useState("bl");
  ...
  const[blClosingFrom,setBlClosingFrom]=useState("");
  ...
  const blFilters={search:blSearch,setSearch:setBlSearch,closingFrom:blClosingFrom,setClosingFrom:setBlClosingFrom,filterCustomer:blFilterCustomer,setFilterCustomer:setBlFilterCustomer,filterStatus:blFilterStatus,setFilterStatus:setBlFilterStatus,page:blPage,setPage:setBlPage};
  ...
  const renderPage=()=>{
    ...
    switch(page){
      ...
      case"arap": return<ARAPSearchPage onOpenARAP={openARAP}/>;
      case"genexp": return<GeneralExpensePage/>;
      case"closing": return<ClosingPage/>;
      case"report": return<ReportsPage/>;
      case"stl": return<PlaceholderPage title="Settlement Ledger" icon="📑" description="Settlement and reconciliation."/>;
