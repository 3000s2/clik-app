...
function DashboardPage({onOpenARAP,onNav}){
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:14,marginBottom:24,flexWrap:"wrap"}}>
      ...
      <Card label="GE Total" value={fmt(d.geTotal)} color={C.text}/>
    </div>
    {/* This month */}
...
function SettlementPage({onOpenARAP}){
  ...
  const[sPage,setSPage]=useState(1);const S_PAGE_SIZE=50;
    ...
    for(const job of jobs){
      const proc=(charges,type)=>{
        ...
        for(const[key,val]of Object.entries(charges)){
          ...
          const slipped=!!charges[key+"_slipped"];
          const vendorInvNum=charges[key+"_vendorInvNum"]||"";const vendorInvDate=charges[key+"_vendorInvDate"]||"";
          const paid=payments.reduce((s,p)=>M.sum(s,p.amount),0);
          let status="ACCRUED",color=C.purple;
          if(paid>=amt){status="REMITTED";color=C.green;}
          else if(paid>0){status="P REMITTED";color=C.orange;}
          else if(slipped){status="SLIPPED";color=C.blue;}
          lines.push({jobId:job.id,bl:job.bl||job.mbl||"",customer:job.customer,code:key,label:key,amount:amt,currency:cur,payee,payments,dueDate,slipped,vendorInvNum,vendorInvDate,status,color,type,paid});
        }
        // Manual AP
        for(const l of (charges._manualAP||[])){
          ...
          lines.push({jobId:job.id,bl:job.bl||job.mbl||"",customer:job.customer,code:l.code,label:l.label||l.code,amount:amt,currency:l.currency||"CAD",payee:l.payee||"",payments,dueDate:l.dueDate||"",slipped,vendorInvNum:l.vendorInvNum||"",vendorInvDate:l.vendorInvDate||"",status,color,type:"AP",paid});
        }
      };
      if(job.cbEnabled)proc(job.cb,"CB");
      if(job.ffEnabled)proc(job.ff,"FF");
  ...
  const filteredAR=arWithInvoice.filter(l=>{
    if(dSearchEntity){const q=dSearchEntity.toLowerCase();if(!(l.customer.toLowerCase().includes(q)||l.jobId.toLowerCase().includes(q)||(l.bl||"").toLowerCase().includes(q)||l.code.toLowerCase().includes(q)))return false;}
    if(filterStatus&&l.status!==filterStatus)return false;
    return true;
  });
  const filteredAP=apLines.filter(l=>{
    if(dSearchEntity){const q=dSearchEntity.toLowerCase();if(!(l.payee.toLowerCase().includes(q)||l.jobId.toLowerCase().includes(q)||(l.bl||"").toLowerCase().includes(q)||l.code.toLowerCase().includes(q)))return false;}
    if(filterStatus&&l.status!==filterStatus)return false;
    return true;
  });
  ...
  return(<div>
    ...
    <div style={{display:"flex",gap:0,marginBottom:20}}>
      {["AR","AP"].map(t=><button key={t} onClick={()=>{setTab(t);setSelected(new Set());setSelectedInvs(new Set());}} style={{padding:"10px 32px",fontSize:13,fontWeight:700,fontFamily:F.display,border:`1px solid ${C.border}`,background:tab===t?C.accent:C.surface,color:tab===t?"#fff":C.textDim,cursor:"pointer",borderRadius:t==="AR"?"8px 0 0 8px":"0 8px 8px 0"}}>{t==="AR"?"AR (Receivable)":"AP (Payable)"}</button>)}
    </div>
    ...
    <div style={{display:"flex",gap:12,marginBottom:16,alignItems:"flex-end",flexWrap:"wrap"}}>
      ...
      <div><label style={S.label}>Search</label><input value={searchEntity} onChange={e=>{setSearchEntity(e.target.value);}} placeholder="ID, BL, Customer, Payee..." style={{fontFamily:F.mono,fontSize:12,padding:"8px 10px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",minWidth:220}}/></div>
    ...
    {loading?<LoadingOverlay/>:tab==="AR"?(
      ...
      arGroups.length===0?<EmptyState text="No AR lines found."/>:arGroups.map(([customer,lines])=>{
        ...
        return(<div key={customer} style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,padding:16,marginBottom:16}}>
          ...
          {jobGroups.map(([jobId,jobLines])=>(
            <div key={jobId} style={{marginBottom:10,borderLeft:`3px solid ${C.accent}30`,paddingLeft:12}}>
              ...
              {jobLines.map(l=>{const lk=lineKey(l);const sel=selected.has(lk)&&l.status==="ACCRUED";
                const invPaid=l.invoice?(l.invoice.payments||[]).reduce((s,p)=>M.sum(s,p.amount),0):0;
                return(<div key={lk} style={{display:"flex",gap:8,alignItems:"center",padding:"4px 0",fontSize:12,flexWrap:"wrap"}}>
                  ...
                  {l.invoice&&invPaid>0&&<span style={{fontSize:9,color:C.green,fontFamily:F.mono}}>paid {fmt(invPaid)}/{fmt(l.invoice.total)}</span>}
          ...
          {(outstandingInvByCustomer[customer]||[]).length>0&&<div style={{marginTop:8,borderLeft:`3px solid ${C.cyan}30`,paddingLeft:12}}>
            <div style={{fontSize:11,fontWeight:700,color:C.textDim,fontFamily:F.display,marginBottom:4}}>Invoices</div>
            {(outstandingInvByCustomer[customer]||[]).map(inv=>{
              ...
              return(<div key={inv.invoiceNumber} style={{display:"flex",gap:8,alignItems:"center",padding:"4px 0",fontSize:12,flexWrap:"wrap",background:isSel?`${C.accent}06`:"transparent"}}>
                <input type="checkbox" checked={isSel} onChange={()=>toggleInvSelect(inv.invoiceNumber)} style={{accentColor:C.accent}}/>
                <span style={{fontFamily:F.mono,fontSize:11,color:C.accent,fontWeight:600,cursor:"pointer",textDecoration:"underline"}} onClick={()=>setViewInvoice(inv)}>{inv.invoiceNumber}</span>
                <span style={{fontSize:10,color:C.textDim}}>{inv.date}</span>
                {inv.dueDate&&<span style={{fontSize:9,color:C.textMuted}}>due {inv.dueDate}</span>}
                <span style={{fontFamily:F.mono,fontSize:12,color:C.text,fontWeight:600}}>{fmt(inv.total)}</span>
                {invPaid>0&&<span style={{fontFamily:F.mono,fontSize:9,color:C.green}}>paid {fmt(invPaid)}</span>}
                <span style={{fontFamily:F.mono,fontSize:9,color:C.red}}>bal {fmt(inv.balance)}</span>
                ...
                {inv.consolidatedFrom&&inv.consolidatedFrom.length>0&&<Badge color={C.purple}>C</Badge>}
                <div style={{display:"flex",gap:4,marginLeft:"auto"}}>
                  <button onClick={()=>openEditInvoice(inv)} title="Edit" style={{background:"none",border:"none",color:C.textDim,cursor:"pointer",fontSize:11}}>✏️</button>
                  <button onClick={()=>openInvPayModal(inv)} title="Pay" style={{background:"none",border:"none",color:C.cyan,cursor:"pointer",fontSize:11}}>💳</button>
                  <button onClick={()=>deleteInvoice(inv)} title="Delete" style={{background:"none",border:"none",color:C.red,cursor:"pointer",fontSize:11}}>🗑</button>
                </div>
              </div>);
            })}
          </div>}
            ...
            <div key={jobId} style={{marginBottom:10,borderLeft:`3px solid ${C.orange}30`,paddingLeft:12}}>
              ...
              {jobLines.map((l,li)=>(
                <div key={l.code+li} style={{display:"flex",gap:8,alignItems:"center",padding:"4px 0",fontSize:12}}>
                  ...
                  {l.vendorInvNum&&<span style={{fontSize:9,color:C.textMuted,fontFamily:F.mono}} title={l.vendorInvDate?"Inv date: "+l.vendorInvDate:""}>📄{l.vendorInvNum}</span>}
                  <span style={{fontSize:9,fontWeight:700,color:l.color,padding:"2px 6px",borderRadius:4,border:`1px solid ${l.color}30`,background:`${l.color}10`}}>{l.status}</span>
...
function GeneralExpensePage({onNav}){
  ...
  const load=async()=>{
    ...
    setLoading(true);
    try{
      const opts={type:"General",search:dSearch,limit:PAGE_SIZE,offset:(page-1)*PAGE_SIZE};
      if(createdMonth){opts.createdFrom=createdMonth+"-01";opts.createdTo=createdMonth+"-31";}
      const list=await window.db.ap.list(opts);
      const cnt=await window.db.ap.count({type:"General",search:dSearch,createdFrom:opts.createdFrom,createdTo:opts.createdTo});
      setItems(list||[]);setTotal(cnt||0);
      setPayeeList([...DB.payees]);
      if(window.db.codes){const cc=await window.db.codes.list();setCustomCodes(cc||[]);}
    }catch(e){await showAlert("Failed to load expenses: "+e.message);}
    setLoading(false);
  };
  useEffect(()=>{setPage(1);},[dSearch,createdMonth]);
...
function ClosingPage({onOpenARAP,onNav}){
  const[jobs,setJobs]=useState([]);const[geItems,setGeItems]=useState([]);const[totalJobs,setTotalJobs]=useState(0);const[totalGEs,setTotalGEs]=useState(0);
  ...
  const load=async()=>{
    if(!window.db)return;
    const opts={limit:PAGE_SIZE,offset:(page-1)*PAGE_SIZE,search:dSearch,createdFrom,createdTo};
    const jList=await window.db.jobs.list(opts);
    const jCount=await window.db.jobs.count({search:dSearch,createdFrom,createdTo});
    setJobs(jList||[]);setTotalJobs(jCount||0);
    const geList=await window.db.ap.list({type:"General",search:dSearch,createdFrom,createdTo,limit:PAGE_SIZE,offset:(page-1)*PAGE_SIZE});
    const geCount=await window.db.ap.count({type:"General",search:dSearch,createdFrom,createdTo});
    setGeItems(geList||[]);setTotalGEs(geCount||0);
    setSelected(new Set());
  };
  useEffect(()=>{setPage(1);},[dSearch,createdFrom,createdTo]);
  useEffect(()=>{load();},[dSearch,createdFrom,createdTo,page]);

  // Combine BLs (server-paginated) + GEs (client-loaded, usually small)
  const allRows=[];
  ...
  const totalItems=totalJobs+totalGEs;
  const totalPages=Math.max(1,Math.ceil(Math.max(totalJobs,totalGEs)/PAGE_SIZE));
  ...
  return(<div>
    ...
    <Pagination page={page} totalPages={totalPages} total={totalItems} onPage={setPage}/>
  </div>);
}
