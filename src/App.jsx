...
const AP_TAX_PROFILES=[
  ...
  {key:"gst",label:"GST 5%",rate:0.05,components:[{type:"GST",rate:0.05,label:"GST (5%)"}]},
  {key:"on",label:"HST 13% (ON)",rate:0.13,components:[{type:"GST",rate:0.05,label:"GST (5%)"},{type:"HST",rate:0.08,label:"HST ON (8%)"}]},
  {key:"atl",label:"HST 15% (NB/NS/NL/PE)",rate:0.15,components:[{type:"GST",rate:0.05,label:"GST (5%)"},{type:"HST",rate:0.10,label:"HST (10%)"}]},
...
const PROVINCE_TO_PROFILE={"Ontario":"on","British Columbia":"bc","Quebec":"qc","Alberta":"gst","Manitoba":"mb","Saskatchewan":"sk","New Brunswick":"atl","Nova Scotia":"atl","Newfoundland and Labrador":"atl","Prince Edward Island":"atl","Northwest Territories":"gst","Yukon":"gst","Nunavut":"gst"};
...
function ARAPDetailPage({job:initialJob,onBack}){
  ...
  useEffect(()=>{
    ...
    for(const[key,val]of Object.entries(charges)){
      ...
      ar.push({code:codeInfo.code,label:codeInfo.label,amount:val,currency,notes,disb,payee,payments,taxPaid,taxPaidComponents,locked:false});
    }
  ...
  const addARLine=(codeInfo)=>{
    ...
    setArLines(p=>[...p,{code:codeInfo.code,label:codeInfo.label,amount:"",currency:"CAD",notes:"",disb:false,payee:"",payments:[],taxPaid:"",taxPaidComponents:[],locked:false}]);setDirty(true);
  };
  // Add AP line
  const addAPLine=(codeInfo)=>{
    if(apLines.some(l=>l.code===codeInfo.code))return;
    setApLines(p=>[...p,{code:codeInfo.code,label:codeInfo.label,amount:"",currency:"CAD",notes:"",payee:"",payments:[],taxPaid:"",taxPaidComponents:[]}]);setDirty(true);
  };
  // Remove lines
  ...
  const saveCharges=async()=>{
    ...
    for(const l of arLines){
      ...
      if(l.disb){charges[l.code+"_disb"]=true;charges[l.code+"_payee"]=l.payee||"";if(M.round(l.taxPaid))charges[l.code+"_taxPaid"]=M.round(l.taxPaid);if(l.taxPaidComponents&&l.taxPaidComponents.length>0)charges[l.code+"_taxPaidComponents"]=l.taxPaidComponents;}
    ...
    const manualAP=apLines.map(l=>({code:l.code,label:l.label,amount:l.amount,currency:l.currency,notes:l.notes,payee:l.payee,payments:l.payments||[],taxPaid:M.round(l.taxPaid),taxPaidComponents:l.taxPaidComponents||[]}));
  ...
  return(<div>
      ...
      <div style={{display:"flex",justifyContent:"flex-end",gap:8,marginTop:16}}>
        ...
        <Button onClick={async()=>{
          ...
          try{
            ...
            delete updData.editItems;delete updData.editDate;delete updData.editDueDate;delete updData.editProvince;delete updData.editCurrency;delete updData.editFxRate;delete updData.editNoTax;delete updData.editCustomTaxRate;delete updData.editCustomTaxLabel;delete updData.editCustomTaxType;
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12,marginBottom:16}}>
        ...
        <div>
          ...
          {!invNoTax&&<div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
            ...
            <span style={{fontSize:10,color:C.textMuted}}>%</span>
            {invCustomTaxRate&&<select value={invCustomTaxType} onChange={e=>setInvCustomTaxType(e.target.value)} style={{fontFamily:F.mono,fontSize:11,padding:"6px 4px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",cursor:"pointer"}}><option value="GST">GST</option><option value="HST">HST</option><option value="PST">PST</option><option value="QST">QST</option></select>}
            {invCustomTaxRate&&<input value={invCustomTaxLabel} onChange={e=>setInvCustomTaxLabel(e.target.value)} placeholder={`Label e.g. ${invCustomTaxType} ${invCustomTaxRate}%`} style={{fontFamily:F.body,fontSize:11,padding:"7px 8px",background:C.bg,border:`1px solid ${C.border}`,borderRadius:6,color:C.text,outline:"none",flex:1}}/>}
    ...
    {viewInvoice&&(()=>{
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(260px,1fr))",gap:12,marginBottom:16,fontSize:13}}>
...
function GeneralExpensePage(){
  ...
  // Form state
  const emptyForm={code:"",label:"",amount:"",currency:"CAD",notes:"",payee:"",closingMonth:"",registeredDate:new Date().toISOString().split("T")[0],paidDate:"",taxPaidComponents:[],memo:""};
  const[form,setForm]=useState(emptyForm);
  const upd=(k,v)=>setForm(p=>({...p,[k]:v}));
  ...
  const startEdit=(item)=>{
    ...
    setForm({code,label,amount:String(item.total||""),currency:item.currency||"CAD",notes:parsed.notes,payee:item.payee||"",closingMonth:parsed.closingMonth,registeredDate:item.date||"",paidDate:item.paidDate||"",taxPaidComponents:item.taxPaidComponents||[],memo:""});
  ...
  return(<div>
    ...
    {showForm&&<div style={{background:C.surface,borderRadius:10,padding:24,border:`1px solid ${C.accent}33`,marginBottom:24}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))",gap:12}}>
        ...
        <div><label style={S.label}>Tax Paid (ITC)</label><TaxRateSelect amount={form.amount} value={form.taxPaidComponents} onChange={v=>upd("taxPaidComponents",v)}/></div>
    ...
    <div style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,overflow:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>
        {["ID","Code","Payee","Amount","Tax Paid","Status","Closing","Registered","Paid Date","Notes",""].map(h=><th key={h} style={S.th}>{h}</th>)}
      </tr></thead>
      <tbody>{items.length===0?<tr><td colSpan={11} style={S.empty}>No general expenses.</td></tr>:items.map(item=>{
        const parsed=parseMemo(item.memo);
        const code=(item.items||[])[0]?.code||"";const label=(item.items||[])[0]?.label||"";
...
function ReportsPage(){
  ...
  const genClosing=async()=>{
    ...
    try{
      ...
      // Aggregate tax paid (input tax credits) by type from closed BL charges + closed GEs
      const taxPaidByType={GST:0,HST:0,PST:0,QST:0};
      const addTPC=comps=>{for(const c of (comps||[])){if(c.type&&c.amount)taxPaidByType[c.type]=M.sum(taxPaidByType[c.type]||0,c.amount);}};
      ...
      for(const ge of closedGEs)addTPC(ge.taxPaidComponents);
      // Distribute paid amounts into matching taxMap rows by type
      for(const[pType,pAmt]of Object.entries(taxPaidByType)){
        if(pAmt<=0)continue;
        // Find first row matching this type
        const matchKey=Object.keys(taxMap).find(k=>taxMap[k].type===pType);
        if(matchKey){taxMap[matchKey].paid=M.sum(taxMap[matchKey].paid,pAmt);}
        else{const label=pType==="GST"?"GST (ITC)":pType==="HST"?"HST (ITC)":pType==="PST"?"PST (ITC)":"QST (ITC)";taxMap[label]={type:pType,collected:0,paid:pAmt};}
      }
