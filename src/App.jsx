...
function DashboardPage({onOpenARAP,onNav}){
  ...
  const load=async()=>{
    ...
    try{
      ...
      const jobs=allJobs||[];const invs=(allInv||[]).filter(i=>!i.consolidatedInto);const ges=(allAp||[]).filter(a=>a.type==="General");
      // Counts
      const totalBLs=jobs.length;const openBLs=jobs.filter(j=>!j.closedMonth).length;const closedBLs=totalBLs-openBLs;
      // AR outstanding
      let arOutstanding=0,arTotal=0;const arByCust={};
      for(const inv of invs){
        const paid=(inv.payments||[]).reduce((s,p)=>M.sum(s,p.amount),0);
        const bal=M.sub(inv.total,paid);arTotal=M.sum(arTotal,inv.total);
        if(bal>0){arOutstanding=M.sum(arOutstanding,bal);arByCust[inv.customer]=M.sum(arByCust[inv.customer]||0,bal);}
      }
      // AP outstanding
      let apOutstanding=0;const apByPayee={};
      for(const job of jobs){
        const proc=ch=>{if(!ch)return;
          for(const[key,val]of Object.entries(ch)){
            ...
            const paid=(ch[key+"_payments"]||[]).reduce((s,p)=>M.sum(s,p.amount),0);
            const bal=M.sub(amt,paid);const payee=ch[key+"_payee"]||"Unknown";
            if(bal>0){apOutstanding=M.sum(apOutstanding,bal);apByPayee[payee]=M.sum(apByPayee[payee]||0,bal);}
          }
          for(const l of (ch._manualAP||[])){
            const amt=M.round(l.amount);if(amt<=0)continue;
            const paid=(l.payments||[]).reduce((s,p)=>M.sum(s,p.amount),0);
            const bal=M.sub(amt,paid);
            if(bal>0){apOutstanding=M.sum(apOutstanding,bal);apByPayee[l.payee||"Unknown"]=M.sum(apByPayee[l.payee||"Unknown"]||0,bal);}
          }
        };
      ...
      const monthBLs=jobs.filter(j=>(j.createdAt||"").startsWith(thisMonth)).length;
      const monthInvs=invs.filter(i=>(i.date||"").startsWith(thisMonth)).length;
      const monthPayments=invs.reduce((s,inv)=>{
        for(const p of (inv.payments||[])){if((p.date||"").startsWith(thisMonth))s=M.sum(s,p.amount);}return s;
      },0);
      // Top customers/payees
      ...
      // Recent BLs
      const recentBLs=jobs.slice(0,8);
      setData({totalBLs,openBLs,closedBLs,arOutstanding,apOutstanding,arTotal,monthBLs,monthInvs,monthPayments,topCust,topPayee,recentBLs,totalInvs:invs.length,totalGEs:ges.length});
...
function SettlementPage({onOpenARAP}){
  ...
  const load=async()=>{
    ...
    try{
      const allJobs=await window.db.jobs.list({limit:9999,offset:0,search:""});
      const allInv=await window.db.invoices.list({limit:9999,offset:0});
      setJobs(allJobs||[]);setInvoices(allInv||[]);
  ...
  return(<div>
    ...
    {loading?<LoadingOverlay/>:tab==="AR"?(
      ...
      arGroups.length===0?<EmptyState text="No AR lines found."/>:arGroups.map(([customer,lines])=>{
        ...
        // Aging by invoice date or createdAt
        ...
        return(<div key={customer} style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,padding:16,marginBottom:16}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>
            ...
            {["Job","Code","Amount","Invoice","Status"].map(h=><th key={h} style={S.th}>{h}</th>)}
          </tr></thead><tbody>
          {lines.map(l=>{const lk=lineKey(l);const sel=selected.has(lk)&&l.status==="ACCRUED";
            return(<tr key={lk} style={{borderBottom:`1px solid ${C.border}`}} onMouseEnter={e=>e.currentTarget.style.background=C.surfaceHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              <td style={S.td}>{l.status==="ACCRUED"&&<input type="checkbox" checked={sel} onChange={()=>toggleSelect(lk)} style={{accentColor:C.accent}}/>}</td>
              <td style={S.td}><IdLink id={l.jobId} onClick={()=>{const j=jobs.find(x=>x.id===l.jobId);if(j&&onOpenARAP)onOpenARAP(j);}}/></td>
              <td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.orange,fontWeight:600}}>{l.code}</span></td>
              <td style={S.td}><span style={{fontFamily:F.mono,fontSize:12}}>{l.currency} {fmt(l.amount)}</span></td>
              <td style={S.td}>{l.invNum?<span style={{fontFamily:F.mono,fontSize:11,color:C.accent,fontWeight:600,cursor:"pointer",textDecoration:"underline"}} onClick={()=>setViewInvoice(l.invoice)}>{l.invNum}</span>:<span style={{fontSize:10,color:C.textMuted}}>—</span>}</td>
              <td style={S.td}><span style={{fontSize:10,fontWeight:700,color:l.color,padding:"2px 8px",borderRadius:4,border:`1px solid ${l.color}30`,background:`${l.color}10`}}>{l.status}</span>
                {l.invoice&&l.status!=="REMITTED"&&<button onClick={()=>openInvPayModal(l.invoice)} title="Record payment" style={{background:"none",border:"none",color:C.cyan,cursor:"pointer",fontSize:12,marginLeft:4}}>💳</button>}
              </td>
            </tr>);
          })}
          </tbody></table>
      ...
      apGroups.length===0?<EmptyState text="No AP lines found."/>:apGroups.map(([payee,lines])=>{
        ...
        return(<div key={payee} style={{background:C.surface,borderRadius:10,border:`1px solid ${C.border}`,padding:16,marginBottom:16}}>
          ...
          <table style={{width:"100%",borderCollapse:"collapse"}}><thead><tr style={{background:C.card}}>
            {["Job","Code","Amount","Due Date","Status","Actions"].map(h=><th key={h} style={S.th}>{h}</th>)}
          </tr></thead><tbody>
          {lines.map((l,li)=>{
            return(<tr key={l.jobId+l.code+li} style={{borderBottom:`1px solid ${C.border}`}} onMouseEnter={e=>e.currentTarget.style.background=C.surfaceHover} onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
              <td style={S.td}><IdLink id={l.jobId} onClick={()=>{const j=jobs.find(x=>x.id===l.jobId);if(j&&onOpenARAP)onOpenARAP(j);}}/></td>
              <td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:C.orange,fontWeight:600}}>{l.code}</span><span style={{fontSize:10,color:C.textDim,marginLeft:4}}>{l.label}</span></td>
              <td style={S.td}><span style={{fontFamily:F.mono,fontSize:12}}>{l.currency} {fmt(l.amount)}</span>{l.paid>0&&<span style={{fontSize:9,color:C.green,marginLeft:4}}>(paid {fmt(l.paid)})</span>}</td>
              <td style={S.td}><span style={{fontFamily:F.mono,fontSize:11,color:l.dueDate?C.text:C.textMuted}}>{l.dueDate||"—"}</span></td>
              <td style={S.td}><span style={{fontSize:10,fontWeight:700,color:l.color,padding:"2px 8px",borderRadius:4,border:`1px solid ${l.color}30`,background:`${l.color}10`}}>{l.status}</span></td>
              <td style={S.td}><div style={{display:"flex",gap:4}}>
                ...
                {l.status!=="REMITTED"&&<button onClick={()=>openAPPayModal(l)} title="Record payment" style={{background:"none",border:"none",color:C.cyan,cursor:"pointer",fontSize:12}}>💳</button>}
              </div></td>
            </tr>);
          })}
          </tbody></table>
