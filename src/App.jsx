...
function BLCreationPage({onOpenARAP,onOpenBLDetail,filters}){
  ...
  const closingMonth=filters.closingMonth;const setClosingMonth=filters.setClosingMonth;
  ...
  const load=async()=>{
    ...
    setLoading(true);
    try{
      const opts={limit:PAGE_SIZE,offset:(page-1)*PAGE_SIZE,search:dSearch,closingMonth};
      const[list,cnt]=await Promise.all([window.db.jobs.list(opts),window.db.jobs.count({search:dSearch,closingMonth})]);
...
function GeneralExpensePage({onNav}){
  ...
  const emptyForm={lineItems:[{...emptyLine}],currency:"CAD",notes:"",payee:"",closingMonth:"",registeredDate:new Date().toISOString().split("T")[0],paidDate:"",taxPaidComponents:[],memo:""};
  const[form,setForm]=useState(emptyForm);
  const upd=(k,v)=>setForm(p=>({...p,[k]:v}));
  ...
  const saveExpense=async()=>{
    ...
    try{
      if(editId){
        ...
        await dbAddAP({id,jobId:"",customer:"",payee:form.payee,date:form.registeredDate,items,total,type:"General",payments:[],dueDate:"",invoiceNum:"",fxRate:0,memo:"closing:"+form.closingMonth+"|"+form.notes,currency:form.currency,paidDate:form.paidDate,taxPaid:tp,taxPaidComponents:tpc});
      }
  ...
  const startEdit=(item)=>{
    ...
    setForm({lineItems,currency:item.currency||"CAD",notes:parsed.notes,payee:item.payee||"",closingMonth:parsed.closingMonth,registeredDate:item.date||"",paidDate:item.paidDate||"",taxPaidComponents:item.taxPaidComponents||[],memo:""});
...
function ClosingPage({onOpenARAP,onNav}){
  ...
  const load=async()=>{
    if(!window.db)return;
    // Load BLs
    const opts={limit:999,offset:0,search:dSearch,closingMonth:closingMonth||""};
...
function ReportsPage({onOpenARAP,onNav}){
  ...
  const genClosing=async()=>{
    ...
    try{
      const allJobs=await window.db.jobs.list({limit:9999,offset:0,search:"",closingMonth:clMonth});
...
${co.notesTerms?`<div style="margin-top:30px;padding-top:16px;border-top:1px solid #ccc"><div style="font-size:11px;font-weight:700;color:#555;margin-bottom:4px">Terms</div><div style="font-size:11px;color:#444;white-space:pre-wrap">${esc(co.notesTerms)}</div></div>`:""}
  ...
  const genApAging=async()=>{
    ...
    try{
      const allJobs=await window.db.jobs.list({limit:9999,offset:0,search:"",closingMonth:""});
  ...
  const genRevByCust=async()=>{
    ...
    try{
      ...
      const custMap={};
      for(const job of (allJobs||[])){
        const cm=(job.closingDate||"").slice(0,7);
...
function AppInner(){
  ...
  const[blClosingMonth,setBlClosingMonth]=useState("");
  ...
  const blFilters={search:blSearch,setSearch:setBlSearch,closingMonth:blClosingMonth,setClosingMonth:setBlClosingMonth,filterCustomer:blFilterCustomer,setFilterCustomer:setBlFilterCustomer,filterStatus:blFilterStatus,setFilterStatus:setBlFilterStatus,page:blPage,setPage:setBlPage};
